...
class member_base extends main{
  ...
  public function login($username,$password){
    ...
    $sql = "SELECT member_id,member_email,member_password,member_type FROM Member WHERE member_email='$username' AND member_password='$password'";
    $query = $this->dbConn->query($sql);
    $rows_mem = $query->num_rows;
    if($rows_mem>0){
      if($this->checkLoginByWebService($username)==true){
        $rs_mem = $query->fetch_assoc();
        $_SESSION["member"]["member_id"] = $rs_mem["member_id"];
        $_SESSION["member"]["member_type"] = $rs_mem["member_type"];
