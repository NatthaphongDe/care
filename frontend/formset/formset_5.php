...
<div class="panel-body">';
  ...
  if($lang == "1"){ $top_form5 = "หัวข้อเรื่องร้องเรียน";}elseif($lang == "2"){ $top_form5 = "Subject of Complaint";}else{ $top_form5 = "หัวข้อเรื่องร้องเรียน";}
  ...
  if($lang == "1"){ $Category_form5 = "ประเภทสินค้า";}elseif($lang == "2"){ $Category_form5 = "Type of Product";}else{ $Category_form5 = "ประเภทสินค้า";}
    $formSet_html .='<div class="col-md-3">'.$Category_form5.'<span style="color:#E74C3C;">*</span></div>
    <div class="col-md-9">
      <select name="prodType_id_IdxFs_'.$formSetId.'" id="sel_category_invite" class="selectpicker form-control sel_category_invite" data-live-search="true" onchange="chk_prod();">
      <option value="">';
      if($lang == "1"){ $chc_form3 = "เลือกประเภทสินค้า";}elseif($lang == "2"){ $chc_form3 = "Please choose Type of Product";}else{ $chc_form3 = "เลือกประเภทสินค้า";}
        $formSet_html .='--- '.$chc_form3.' ---</option>';

        function getProdType($lv,$ref_id,$ref_name){
            global $case_cls;
            global $lang;
            $i=0;
            foreach($case_cls->prodTypeListMutiLv($lv,$ref_id) as $prod_type){
              if($lv==1){
                $option .= '<optgroup>';
              }
              ...
              $option .= '<option '.$disabled.' value="'.$prod_type["prodType_id"].'" rel="'.$prod_type["prodType_other_flag"].'"
              data-content="<span class=\'txt\' style=\'padding-left:'.(20*($lv-1)).'px\'>'.$arrow.'<h style=\'display:none;\'>'.$ref_name_real.'</h>'.$prodType_name.'</span>" >
                          '.$prodType_name.'
                        </option>';
              if($prod_type["prodType_sublist"]>0){
                $n_lv = $lv+1;
          ...
          $formSet_html .= getProdType(1,null);
  ...
  if($lang == "1"){ $top_form5s = "โปรดระบุ";}elseif($lang == "2"){ $top_form5s = "Please specify";}else{ $top_form5s = "โปรดระบุ";}
    ...
    <div class="col-md-4"><input type="text" class="form-control prodType_other" name="prodType_other_IdxFs_'.$formSetId.'"></div>
    </div>
  <div class="row div_history_invite">';
  if($lang == "1"){ $hoc_form5 = "ความเป็นมาของประเด็นเรื่องร้องเรียน";}elseif($lang == "2"){ $hoc_form5 = "History of the Issue";}else{ $hoc_form5 = "ความเป็นมาของประเด็นเรื่องร้องเรียน";}
  ...
  if($lang == "1"){ $dv_form5 = "มูลค่าความเสียหาย (ระบุเป็นตัวเลข)";}elseif($lang == "2"){ $dv_form5 = "Damage (in currency value)";}else{ $dv_form5 = "มูลค่าความเสียหาย (ระบุเป็นตัวเลข)";}
  ...
  if($lang == "1"){ $cn_form5 = "ความต้องการของผู้ร้องเรียน";}elseif($lang == "2"){ $cn_form5 = "Complainant Demands";}else{ $cn_form5 = "ความต้องการของผู้ร้องเรียน";}
  ...
  if($lang == "1"){ $acd_form5 = "แนบเอกสารประกอบการร้องเรียน";}elseif($lang == "2"){ $acd_form5 = "Documents of Evidence";}else{ $acd_form5 = "แนบเอกสารประกอบการร้องเรียน";}
    ...
    <input type="file" class="form-control file_box" name="file_invite[]" multiple accept=".jpg,.jpeg,.png,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.pdf,.zip,.rar,.txt"><button type="button" class="btn btn-default btn_browse_file">Browse</button>';
    if($lang == "1"){ $acda_form5 = "สามารถเลือกไฟล์ได้ ไม่เกินครั้งละ 20 ไฟล์ และขนาดต้องไม่เกิน 10 MB ต่อไฟล์ <br> (ไฟล์ .jpg,.jpeg,.png,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.pdf,.zip,.rar,.txt เท่านั้น)";}elseif($lang == "2"){ $acda_form5 = "20 files maximum per turn, with file size limit of 50 mb each <br> (only .jpg,.jpeg,.png,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.pdf,.zip,.rar,.txt)";}else{ $acda_form5 = "สามารถเลือกไฟล์ได้ ไม่เกินครั้งละ 20 ไฟล์ และขนาดต้องไม่เกิน 10 MB ต่อไฟล์ <br> (ไฟล์ .jpg,.jpeg,.png,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.pdf,.zip,.rar,.txt เท่านั้น)";}
...
<div class="panel-body">';
      ...
      if($lang == "1"){ $chc_form3 = "เลือกประเภทสินค้า";}elseif($lang == "2"){ $chc_form3 = "Please choose Type of Product";}else{ $chc_form3 = "เลือกประเภทสินค้า";}
        ...
        function getProdType($lv,$ref_id,$ref_name){
            ...
            foreach($case_cls->prodTypeListMutiLv($lv,$ref_id) as $prod_type){
              ...
              $ref_name_real = $ref_name."/".$prod_type["prodType_name"];
              $option .= '<option value="'.$prod_type["prodType_id"].'" '.($_POST["prodType_id"]==$prod_type['prodType_id']?'selected':'').'
              rel="'.$prod_type["prodType_other_flag"].'" data-content="<span class=\'txt\' style=\'padding-left:'.(20*($lv-1)).'px\'>'.$arrow.'<h style=\'display:none;\'>'.$ref_name_real.'</h>'.$prodType_name.'</span>" >
                          '.$prodType_name.'
                        </option>';
              if($prod_type["prodType_sublist"]>0){
                $n_lv = $lv+1;
  ...
  if($lang == "1"){ $top_form5s = "โปรดระบุ";}elseif($lang == "2"){ $top_form5s = "Please specify";}else{ $top_form5s = "โปรดระบุ";}
    $formSet_html .='<div class="col-md-3">'.$top_form5s.'<span style="color:#E74C3C;">*</span></div>
    <div class="col-md-4"><input type="text" class="form-control prodType_other" name="prodType_other_IdxFs_'.$formSetId.'" value="'.$_POST['prodType_other'].'"></div>
    </div>
  <div class="row div_history_invite">';
  if($lang == "1"){ $hoc_form5 = "ความเป็นมาของประเด็นเรื่องร้องเรียน";}elseif($lang == "2"){ $hoc_form5 = "History of the Issue";}else{ $hoc_form5 = "ความเป็นมาของประเด็นเรื่องร้องเรียน";}
    $formSet_html .='<div class="col-md-3">'.$hoc_form5.'<span style="color:#E74C3C;">*</span></div>
