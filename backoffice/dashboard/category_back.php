...
if($search_cat==0){
  $where_year = " AND case_create_datetime like '%".$year."%' ";
  ...
  if($year_type_cat=='2'){
    ...
    $sp = $issue_year."-09-30";
    $where_year = " AND case_create_datetime >= '$st'  AND case_create_datetime <= '$sp' ";
  }else{
    if ($month!='') {
      if($month=='1'){$mon = '-01-';}
      if($month=='2'){$mon = '-02-';}
      ...
      if($month=='11'){$mon = '-11-';}
      if($month=='12'){ $mon = '-12-';}
      $where_year = "AND case_create_datetime like '%".$mon."%'  ";
      ...
      $where_year = "AND case_create_datetime like '%".$issue_year."%'  ";
      ...
      $where_year = " AND case_create_datetime >= '$startDateY'  AND case_create_datetime <= '$stopDateY' ";
    }
  }
}
// echo $where_year;
...
if($query_lv1->num_rows>0){
  ...
  <div class="gp_compType_name">
    ...
    while ( $re_edit = $query_lv1->fetch_assoc()) {
      ...
      $sql_lv2 = "SELECT * FROM `Complaint_Type_Sub1` WHERE `compType_id` = '".$re_edit[compType_id]."' AND compTypeSub1_status = 0";
      $query_lv2 = $conn->query($sql_lv2);
      if($query_lv2->num_rows>0){
        while ( $re_lv2 = $query_lv2->fetch_assoc()) {

          ?>
          <div class="display_block  col-md-10 col-sm-10 col-xs-10">
            <div class="box_cat_color" style="background:<?=$date[$color];?>"></div>
            <?=$re_lv2['compTypeSub1_name']; ?>
          </div>
          <div class="total_cat col-md-2 col-sm-2 col-xs-2">
            <?php

            $sql_count = "SELECT Count(c.compTypeSub1_id) AS compType_id FROM  `Case` as c
            left join  Complaint_Type on Complaint_Type.compType_id = c.compType_id
            WHERE c.compTypeSub1_id = '".$re_lv2['compTypeSub1_id']."'
            ...
            while ( $re_count =   $query_count->fetch_assoc()) {
              ...
              $name =  $re_edit['compTypeSub1_name'];
          ...
          $sql_edit = " SELECT * FROM `Complaint_Type`
                        WHERE `compType_status` = 0
                        AND  compType_id = $re_edit[compType_id]  ";
...
if($_SESSION["admin"]["empSection"]!=0){
  $sql_empSection = "  AND  casePrt_section = '".$_SESSION["admin"]["empSection"]."' ";
}
    $sql_edit = "SELECT casePrt_id,casePrt_name,casePrt_img_path FROM `Case_Priority` WHERE `casePrt_status` = 0 
    $sql_empSection
    ";
    $query_edit = $conn->query($sql_edit);
    if($query_edit->num_rows>0){
