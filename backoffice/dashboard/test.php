...
// echo $date[$color];
    ...
    <div class="total_cat">
      ...
      while ( $re_count =   $query_count->fetch_assoc()) {
        echo  $Waiting = $re_count['compType_id'];
      }
      ?></div><?php
        echo "<br>";
            $color++;
 ...
 ?><div class="gp_compType_name">
   <div class="db_lbl_title div_pti">
     <lable class="title_case">Prioroty</lable>
...
<script type="text/javascript">
  ...
  Highcharts.chart('container', {
    ...
    colors: ['#7E57C2', '#F06292', '#4DB6AC', '#8D6E63','#11EA11' ,'#FF0000' ,'#E5A505','#0A16EF','#2E8B57','#FFE4E1'],
    // colors: ['<?php echo $date[0]; ?><?php for($i=1;$i<count($date);$i++){ ?>, '<?php echo $date[$i]; ?>'<?php } ?>'],
    ...
    series: [{
      ...
      data: [
        <?php
        $sql_count = "SELECT  Case_id FROM  `Case` as c
                      left join  Complaint_Type on Complaint_Type.compType_id = c.compType_id
                      ...
                      AND  Complaint_Type.compType_section = '".$_SESSION["admin"]["empSection"]."'
                       ";
        $query_count = $conn->query($sql_count);
        $num_rows_count = $query_count->num_rows;
        $sql_edit = "SELECT compType_name,compType_id FROM `Complaint_Type` WHERE `compType_status` = 0 ";
        $query_edit = $conn->query($sql_edit);
        // if($query_edit->$num_rows>0){
        while ( $re_edit = $query_edit->fetch_assoc()) {
            ...
            $sql_count = "SELECT Count(c.compType_id) AS compType_id FROM  `Case`  as c
                          left join  Complaint_Type on Complaint_Type.compType_id = c.compType_id
                          WHERE c.compType_id = '".$re_edit['compType_id']."' $where_year
                          AND  Complaint_Type.compType_section = '".$_SESSION["admin"]["empSection"]."'   ";
            $query_count = $conn->query($sql_count);

            while ($re_count = $query_count->fetch_assoc()) {

                $Waiting = $re_count['compType_id'];
    ...
    title: {
      text: '<span class="total_txt">Total Case</span><br><span class="total_txt_sun" style="  font-weight: lighter;font-family:kanit;font-size: 40px;"><?=$num_rows_count;?></span>',
      align: 'center',
