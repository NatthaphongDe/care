...
class case_base extends main
{
  ...
  public function caseStatusList($type)
  {
    ...
    $case_status_close = array('1' => 'ตกลงกันได้', '2' => 'ผู้ร้องดำเนินการต่อ', '3' => 'กรมไม่สามารถดำเนินการได้ ');
  ...
  // --ฟังก์ชั่นเรียกข้อมูล blacklist --//
  public function blacklistInfo($text)
  {
    $blacklistArrObj = array();

    $where_text = '';
    if ($text != '') {
      // $where_text = "AND (cpr_numbertrade = '$text' OR cpr_companyname like '%".$text."%' OR cpr_contact_person like '%".$text."%')";
      $sql = "SELECT `cpr_id`, `cpr_numbertrade`, `cpr_companyname`, `cpr_address`, `cpr_contact_person` , IF(reliable!=0, reliable, cpr_reliable) AS reliable, c.complnt_name  
            FROM `Corporate` cp
            LEFT JOIN `Case` c on cp.cpr_companyname = c.complnt_name
            WHERE c.case_status = 3  AND (cpr_numbertrade = '$text' OR cpr_companyname like '%" . $text . "%' OR cpr_contact_person like '%" . $text . "%') GROUP BY cpr_companyname";
      $query = $this->dbConn->query($sql);
      if ($query->num_rows > 0) {
        while ($result = $query->fetch_assoc()) {
          $provArr["cpr_id"] = $result["cpr_id"];
          $provArr["complnt_trade_number"] = $result["cpr_numbertrade"];
          $provArr["complnt_name"] = $result["cpr_companyname"];
          $provArr["complnt_contact_address"] = $result["cpr_address"];
          $provArr["complnt_contact_name"] = $result["cpr_contact_person"];
          $provArr["reliable"] = $result["reliable"];
          array_push($blacklistArrObj, $provArr);
        }
        return $blacklistArrObj;
      }
    } else {
      return NULL;
    }
  ...
  // --ฟังก์ชั่นเรียกข้อมูล blacklist Case --//
  public function blacklistCase($taxID, $compName)
  {
    $blacklistArrObj = array();

    if ($taxID != '') {
      $where_text = " AND complnt_trade_number = '$taxID'";
    } else {
      $where_text = " AND complnt_name = '$compName'";
    }

    $sql = "SELECT case_id, caseDtl_title, reliable FROM `Case` WHERE case_status = 3 $where_text ORDER BY case_id DESC";
    $query = $this->dbConn->query($sql);
    if ($query->num_rows > 0) {
      while ($result = $query->fetch_assoc()) {
        $provArr["case_id"] = $result["case_id"];
        $provArr["caseDtl_title"] = $result["caseDtl_title"];
        $provArr["reliable"] = $result["reliable"];
        array_push($blacklistArrObj, $provArr);
      }
      return $blacklistArrObj;
  ...
  // --ฟังก์ชั่นเรียกข้อมูล blacklist Reliable --//
  public function blacklistReliable($taxID, $compName)
  {
    $blacklistArrObj = array();
    ...
    $sql = "SELECT MAX(case_id) AS case_id FROM `Case` WHERE case_status = 3 $where_text";
    $query = $this->dbConn->query($sql);
    if ($query->num_rows > 0) {
      $result = $query->fetch_assoc();

      $sql1 = "SELECT case_id, caseDtl_title, reliable FROM `Case` WHERE case_id = '$result[case_id]'";
      $query1 = $this->dbConn->query($sql1);
      if ($query1->num_rows > 0) {
        while ($result1 = $query1->fetch_assoc()) {
          $blacklistArrObj = $result1;
        }
      }
      return $blacklistArrObj;
    }
  ...
  // --ฟังก์ชั่นบันทึกข้อมูล แบบสอบถามความพึงพอใจ --//
  ...
  public function CountryData_List($country_id)
  {
    $countryData = array();
    $sql = "SELECT * FROM `Country` WHERE `id` = '$country_id'";
    $query = $this->dbConn->query($sql);
    while ($result = $query->fetch_assoc()) {
      array_push($countryData, $result);
    }
    return $countryData;
  }
  ...
  public function chkCompType()
  {
    ...
    if ($this->compType_id != "" && $this->compTypeSub1 != "" && $this->compTypeSub2 == "") {
      ...
      //   echo '<br>sqlForm.: '.$sqlForm;
      //   echo '<br>numForm.: '.$numForm;
      // }
      if ($numForm > 0) {
        $status_response = "02";
  ...
  {
    if (isset($_SESSION["admin"]["login_as"]) && $_SESSION["admin"]["login_as"] == 1) {
      $caseAsign_createBy_id = 7;
    } else {
      $caseAsign_createBy_id = $this->admin_id;
    }
    $caseAsign_createBy_id = $this->admin_id;
    $sql = "INSERT
              INTO
...
class case_list extends case_base
{
  ...
  public function getCaseList($post)
  {
    ...
    if ($_SESSION["admin"]["empId"] >= 71 && $_SESSION["admin"]["empId"] <= 85) {
      $case_arr = array();
      $emp_id = $_SESSION["admin"]["empId"];
      $sql_caseAssign = "SELECT ca.`case_id`, ca.`caseCh_id`, em.dept_id,  em.emp_id FROM `Case` ca
                          LEFT JOIN Case_Channel ch ON ca.caseCh_id = ch.caseCh_id 
                          LEFT JOIN Department dp ON dp.caseCh_id = ch.caseCh_id 
                          LEFT JOIN Employee em  ON em.dept_id = dp.dept_id 
                          WHERE em.emp_id = '$emp_id'
                          ORDER BY ca.`case_id` DESC";
      $query_caseAssign = $this->dbConn->query($sql_caseAssign);
      $count_case = $query_caseAssign->num_rows;
      while ($result = $query_caseAssign->fetch_assoc()) {
        $case_id = $result['case_id'];
        $sql_case = "SELECT * FROM `Case` WHERE `case_id` = '$case_id'";
        $query_case = $this->dbConn->query($sql_case);
        while ($result_case = $query_case->fetch_assoc()) {
          $page_redirect_ext = "case_detail";
          $page_redirect = "index.php?page=" . $page_redirect_ext . "&caseId=" . $result_case["case_id"];
          $case_temparr = array();
          $case_temparr["caseId"] = '<a href="' . $page_redirect . '">' . $result_case["case_id"] . '</a>';
          $case_temparr["subject"] = '<a href="' . $page_redirect . '">' . $result_case["caseDtl_title"] . '</a>';

          // $case_col_arr["subject"] = '<a href="'.$page_redirect.'">'.$case_col_arr["subject"].'</a>';
          // $case_temparr['caseId'] = $result_case['case_id'];
          // $case_temparr['subject'] = $result_case['caseDtl_title'];
          $case_temparr['applnt'] = $result_case['applntOrg_name'];
          $case_temparr['complnt'] = $result_case['complnt_name'];
          $case_temparr['assign'] = $result['emp_firstname'] . " " . $result['emp_lastname'];
          $case_temparr['office'] = $result_case['office_id'];
          $case_temparr['date'] = date("d/m/Y", strtotime($result_case['case_create_datetime']));
          $case_temparr['status'] = '<img src="img/ico_case_status_' . $result_case["case_status"] . '.png" class="img-status c" />';
          array_push($case_arr, $case_temparr);
        }
      }
    ...
    if ($this->admin_section != "0") {
      ...
      $sql_case_condition = "WHERE cmp_t.compType_section IN (1,2) $office ";
      $sql_case_condition_check = "WHERE cmp_t.compType_section IN (1,2) $office ";
    }
    ...
    if ($this->admin_position == 6) {
      // $sql_case_condition .= " AND c.office_id = '$this->admin_dept' ";
    }
    // echo $sql_case_condition;
    if ($this->admin_section == "1") {
      if ($post->prod_type != "") {
    ...
    //   $sql_case_condition .= " AND (ch.caseCh_id = '$this->admin_dept' OR ch.caseCh_ref_id = '$this->admin_dept')  ";
    // }else{
    if ($post->channel != "") {
      $sql_case .= "LEFT JOIN  `Case_Channel` ch ON (c.caseCh_id=ch.caseCh_id) ";
    ...
    if ($post->text != "") {
      $sql_case .= "LEFT JOIN  `Product_Type` prod_t ON (c.prodType_id=prod_t.prodType_id) ";
      $sql_case .= "LEFT JOIN  `Case_Channel` ch ON (c.caseCh_id=ch.caseCh_id) ";
      $sql_case .= "LEFT JOIN (SELECT ag.case_id, ag.emp_id, em.emp_firstname, em.emp_lastname FROM `Case_Assign` ag LEFT JOIN `Employee` em ON (ag.emp_id=em.emp_id)) asign ON (c.case_id=asign.case_id) ";
      $sql_case .= "LEFT JOIN (SELECT fv.case_id, fv.fieldset_id, fv.fieldset_value, fs.fieldset_name FROM `Field_Values` fv LEFT JOIN Field_Set fs ON fv.fieldset_id = fs.fieldset_id) field ON (c.case_id=field.case_id)";
      $sql_case .= "LEFT JOIN  `Country` ap_c ON (c.applnt_country_id=ap_c.id) ";
      $sql_case .= "LEFT JOIN  `Country` cmp_c ON (c.complnt_country_id=cmp_c.id) ";
      $sql_case .= "LEFT JOIN  `Country` apo_c ON (c.applntOrg_country_id=apo_c.id) ";
      $sql_case_condition .= "
        AND ( c.case_id = '" . (int)$post->text . "'
              ...
              OR (
                (c.complnt_type!=0 AND REPLACE(c.applntOrg_trade_number, '-', '') like CONCAT('%', REPLACE('$post->text', '-', ''), '%'))
                OR (c.complnt_type=0 AND REPLACE(c.applnt_ident, '-', '') like CONCAT('%', REPLACE('$post->text', '-', ''), '%'))
             )
             OR REPLACE(c.complnt_trade_number, '-', '') like  CONCAT('%', REPLACE('$post->text', '-', ''), '%')
             OR prod_t.prodType_name like '%$post->text%'
             OR prod_t.prodType_name_en like '%$post->text%'
             OR ch.caseCh_name like '%$post->text%'
             OR asign.emp_firstname like '%$post->text%'
             OR asign.emp_lastname like '%$post->text%'
             OR cmp_t.compType_name like '%$post->text%'
             OR cmp_t.compType_name_en like '%$post->text%'
             OR c.caseDtl_derivation like '%$post->text%'
             OR c.caseDtl_complnt_need like '%$post->text%'
             OR field.fieldset_value like '%$post->text%'
             OR ap_c.name like '%$post->text%'
             OR ap_c.name_th like '%$post->text%'
             OR cmp_c.name like '%$post->text%'
             OR cmp_c.name_th like '%$post->text%'
             OR apo_c.name like '%$post->text%'
             OR apo_c.name_th like '%$post->text%'
             OR (field.fieldset_value = (SELECT prov_id FROM Province WHERE prov_name like '%$post->text%') and field.fieldset_id IN (90, 11, 26))
    ...
    while ($rs_case_list = $query_case->fetch_assoc()) {
      ...
      $cp_status_bw = $this->check_list_blacklist_watchlist($rs_case["case"],1);
      /* print_r($cp_status_bw);
      exit(); */
      /* if ($rs_case["case"]['case_id'] == '1995') {
        print_r( $cp_status_bw);
        exit();
      } */
      $rp_status_bw = $this->check_list_blacklist_watchlist($rs_case["case"],2);
      ...
      if ($rs_case["case"]["applnt_status"] == 1) {
        ...
        if ($rs_case["case"]["applnt_type"] != 0) {
          $case_col_arr["applnt"] = '<label class="label-width-full label-bg-gray">' . (($rs_case["case"]["applntOrg_name"] == "") ? $rs_case["case"]["applnt_firstname"] . " " . $rs_case["case"]["applnt_lastname"] : $rs_case["case"]["applntOrg_name"]) . '</label>';
      ...
      if ($rs_case["case"]["applntOrg_country_id"] != 0) {
        ...
        if ($rs_case["case"]["applnt_country_id"] != 0) {
          ...
          $case_col_arr["applnt"] .= "<i class='ico-flag' style='background-image: url(img/flags/TH-32.png);' data-toggle='tooltip' data-placement='bottom' data-html='true' title data-original-title='ประเทศไทย'></i>";
        }
      }
      ...
      if ($rs_case["case"]["applnt_valid_dbd"] == 2) {
        $case_col_arr["applnt"] .= '<i class="ico-dbd ico-dbd-2"></i>';
      }
      if ($rs_case["case"]["applnt_backlist"] == 2) {
        $case_col_arr["applnt"] .= '<i class="ico-backlist ico-backlist-2"></i>';
      }
      if($cp_status_bw['status'] == 3){
        $case_col_arr["applnt"] .= '
        <div class="icon-container-bl">
          <i class="ico-bl" ></i>
          <span class="font-bw">'.$cp_status_bw['count']['blacklist'].'</span>
        </div>
        <div class="icon-container-wl">
          <i class="ico-wl" ></i>
          <span class="font-bw">'.$cp_status_bw['count']['watchlist'].'</span>
        </div>';
      }else if($cp_status_bw['status'] == 2){
        $case_col_arr["applnt"] .= '<div class="icon-container-bl">
          <i class="ico-bl" ></i>
          <span class="font-bw">'.$cp_status_bw['count']['blacklist'].'</span>
        </div>';
      }else if($cp_status_bw['status'] == 1){
        $case_col_arr["applnt"] .= '<div class="icon-container-wl">
          <i class="ico-wl" ></i>
          <span class="font-bw">'.$cp_status_bw['count']['watchlist'].'</span>
        </div>';
      }
      if ($rs_case["case"]["applnt_valid_ditp"] == 1 && $rs_case["case"]["applnt_valid_ditp_org"] != "") {
        $case_col_arr["applnt"] .= '<i class="ico-ditp ico-ditp-' . $rs_case["case"]["applnt_valid_ditp_org"] . '"></i>';
      }
      if ($rs_case["case"]["applnt_valid_ditp"] == 2) {
        if ($rs_case["case"]["applnt_valid_ditp_org"] == "non member") {
          $case_col_arr["applnt"] .= '<i class="ico-ditp ico-ditp-non-member"></i>';
        } else {
          $case_col_arr["applnt"] .= '<i class="ico-ditp ico-ditp-2"></i>';
        }
      }
      ...
      if ($rs_case["case"]["complnt_valid_dbd"] == 2) {
        $case_col_arr["complnt"] .= '<i class="ico-dbd ico-dbd-2"></i>';
      }
      /* if ($rs_case["case"]["complnt_backlist"] == 1) {
        $case_col_arr["complnt"] .= '<i class="ico-backlist ico-backlist-1"></i>';
      } */

      if($rp_status_bw['status'] == 3){
        $case_col_arr["complnt"] .= '
        <div class="icon-container-bl">
          <i class="ico-bl" ></i>
          <span class="font-bw">'.$rp_status_bw['count']['blacklist'].'</span>
        </div>
        <div class="icon-container-wl">
        <i class="ico-wl" ></i>
        <span class="font-bw">'.$rp_status_bw['count']['watchlist'].'</span>
      </div>';
      }else if($rp_status_bw['status'] == 2){
        $case_col_arr["complnt"] .= '<div class="icon-container-bl">
          <i class="ico-bl" ></i>
          <span class="font-bw">'.$rp_status_bw['count']['blacklist'].'</span>
        </div>';
      }else if($rp_status_bw['status'] == 1){
        $case_col_arr["complnt"] .= '<div class="icon-container-wl">
          <i class="ico-wl" ></i>
          <span class="font-bw">'.$rp_status_bw['count']['watchlist'].'</span>
        </div>';
      }
      ...
      if ($rs_case["case"]["complnt_valid_ditp"] == 2) {
        if ($rs_case["case"]["complnt_valid_ditp_org"] == "non member") {
          $case_col_arr["complnt"] .= '<i class="ico-ditp ico-ditp-non-member"></i>';
        } else {
          $case_col_arr["complnt"] .= '<i class="ico-ditp ico-ditp-2"></i>';
        }
      }
        ...
        if ($datatime_diff["days"] > 0 && $rs_case["case"]["case_opened_datetime"] != "" && $datatime_diff["days"] > $rs_case["case"]["case_compType_duration"]) { //ถ้าเกิน $this->day_overdue_case วัน
          if ($rs_case["case"]["case_disKPI_status"] == 1) { //ถ้ามีการ Dis KPI
            ...
            if ($datatime_diff["days"] > $rs_case["case"]["case_compType_duration"]) {
              ...
              $statusCase = '<img src="img/ico_case_status_3.png" class="img-status x" />';
            }
          }
        } else { //ถ้าไม่เกิน $this->day_overdue_case วัน
          $statusCase = '<img src="img/ico_case_status_' . $rs_case["case"]["case_status"] . '.png" class="img-status c" />';
  ...
  public function check_list_status_bw($result_bw){
    $data_bw = array();

    if(in_array(1,$result_bw['cn_1']) && in_array(2,$result_bw['cn_1'])){
      array_push($data_bw,3);
    }else if(!in_array(1,$result_bw['cn_1']) && in_array(2,$result_bw['cn_1'])){
      array_push($data_bw,2);
    }else if(in_array(1,$result_bw['cn_1']) && !in_array(2,$result_bw['cn_1'])){
      array_push($data_bw,1);
    }else{
      array_push($data_bw,0);
    }
    /* if(in_array(1,$result_bw['cnt']) && in_array(2,$result_bw['cnt'])){
      array_push($data_bw,3);
    }else if(!in_array(1,$result_bw['cnt']) && in_array(2,$result_bw['cnt'])){
      array_push($data_bw,2);
    }else if(in_array(1,$result_bw['cnt']) && !in_array(2,$result_bw['cnt'])){
      array_push($data_bw,1);
    }else{
    ...
    if(in_array(1,$result_bw['cn']) && in_array(2,$result_bw['cn'])){
      array_push($data_bw,3);
    }else if(!in_array(1,$result_bw['cn']) && in_array(2,$result_bw['cn'])){
      array_push($data_bw,2);
    }else if(in_array(1,$result_bw['cn']) && !in_array(2,$result_bw['cn'])){
      array_push($data_bw,1);
    }else{
    ...
    /* if(in_array(1,$result_bw['ccn']) && in_array(2,$result_bw['ccn'])){
      array_push($data_bw,3);
    }else if(!in_array(1,$result_bw['ccn']) && in_array(2,$result_bw['ccn'])){
      array_push($data_bw,2);
    }else if(in_array(1,$result_bw['ccn']) && !in_array(2,$result_bw['ccn'])){
      array_push($data_bw,1);
    }else{
    ...
    if(in_array(1,$result_bw['ctn']) && in_array(2,$result_bw['ctn'])){
      array_push($data_bw,3);
    }else if(!in_array(1,$result_bw['ctn']) && in_array(2,$result_bw['ctn'])){
      array_push($data_bw,2);
    }else if(in_array(1,$result_bw['ctn']) && !in_array(2,$result_bw['ctn'])){
      array_push($data_bw,1);
    }else{
    ...
    if(in_array(1,$result_bw['ae']) && in_array(2,$result_bw['ae'])){
      array_push($data_bw,3);
    }else if(!in_array(1,$result_bw['ae']) && in_array(2,$result_bw['ae'])){
      array_push($data_bw,2);
    }else if(in_array(1,$result_bw['ae']) && !in_array(2,$result_bw['ae'])){
      array_push($data_bw,1);
    }else{
    ...
    if(in_array(1,$result_bw['at']) && in_array(2,$result_bw['at'])){
      array_push($data_bw,3);
    }else if(!in_array(1,$result_bw['at']) && in_array(2,$result_bw['at'])){
      array_push($data_bw,2);
    }else if(in_array(1,$result_bw['at']) && !in_array(2,$result_bw['at'])){
      array_push($data_bw,1);
    }else{
    ...
    if(in_array(1,$result_bw['ctp']) && in_array(2,$result_bw['ctp'])){
      array_push($data_bw,3);
    }else if(!in_array(1,$result_bw['ctp']) && in_array(2,$result_bw['ctp'])){
      array_push($data_bw,2);
    }else if(in_array(1,$result_bw['ctp']) && !in_array(2,$result_bw['ctp'])){
      array_push($data_bw,1);
    }else{
    ...
    */
    if(in_array(1,$result_bw['fl']) && in_array(2,$result_bw['fl'])){
      array_push($data_bw,3);
    }else if(!in_array(1,$result_bw['fl']) && in_array(2,$result_bw['fl'])){
      array_push($data_bw,2);
    }else if(in_array(1,$result_bw['fl']) && !in_array(2,$result_bw['fl'])){
      array_push($data_bw,1);
    }else{
    ...
    /* print_r($data_bw);
    exit(); */
    if(in_array(3,$data_bw) || (in_array(1,$data_bw) && in_array(2,$data_bw))){
      $status = 3;
    }else if(!in_array(1,$data_bw) && in_array(2,$data_bw) && !in_array(3,$data_bw)){
      $status = 2;
    }else if(in_array(1,$data_bw) && !in_array(2,$data_bw) && !in_array(3,$data_bw)){
      $status = 1;
    }else{
  ...
  public function check_list_blacklist_watchlist($data,$formSetId){
    /* print_r($data);
    exit(); */
    ...
    $case_feild = array();
    $count_name = ['number','company','name','tel','email'];
    $count_bw = array();
    foreach ($count_name as $key => $value) {
      $count_bw[$value]['blacklist'] = 0;
      $count_bw[$value]['watchlist'] = 0;
    }
    $blacklist_case = array();
    $watchlist_case = array();
    $sql = "SELECT * FROM `Field_Values` a
              LEFT JOIN `Field_Set` b ON (a.fieldset_id=b.fieldset_id)
    ...
    $query = $this->dbConn->query($sql);
    while ($rs_feild = $query->fetch_assoc()) {
      $result_feild[$rs_feild["fieldset_name"]] = $rs_feild["fieldset_value"];
    }
    ...
    if($formSetId == 1 ){
      ...
      /* if ($result_feild['applnt_ident'] != ''  && $result_feild['applnt_ident'] != '-') {
        $text = $result_feild['applnt_ident'];
        $sql = "SELECT * FROM `Case` WHERE case_status = 3  AND REPLACE(complnt_trade_number, '-', '') = REPLACE('$text', '-', '') ";
        $query = $this->dbConn->query($sql);
        if ($query->num_rows > 0) {
          $result_bw['cnt'] = array();
          while ($result = $query->fetch_assoc()) {
            array_push($result_bw['cnt'], $result['reliable']);
            if ($result['reliable'] == 2) {
              array_push($blacklist_case,$result['case_id']);
              $count_bw['anum']['blacklist'] = $count_bw['anum']['blacklist'] + 1;
            }
            if( $result['reliable'] == 1){
              array_push($watchlist_case,$result['case_id']);
              $count_bw['anum']['watchlist'] = $count_bw['anum']['watchlist'] + 1;
            }
          }
        }else{
          $result_bw['cnt'] = 0;
      ...
      if ($result_feild['applnt_email'] != '' ) {
        $text = $result_feild['applnt_email'];
        if($text != "no@no.com" && $text != "No@no.com" && $text != "No@No.com" && strtolower($text) != "none@none.com" ){
          $sql = "SELECT * FROM `Case` WHERE case_status = 3  AND complnt_contact_email = '$text' ";
          $query = $this->dbConn->query($sql);
          if ($query->num_rows > 0) {
            $result_bw['ae'] = array();
            while ($result = $query->fetch_assoc()) {
              array_push($result_bw['ae'], $result['reliable']);
              if ($result['reliable'] == 2) {
                array_push($blacklist_case,$result['case_id']);
                $count_bw['email']['blacklist'] = $count_bw['email']['blacklist'] + 1;
              }
              if( $result['reliable'] == 1){
                array_push($watchlist_case,$result['case_id']);
                $count_bw['email']['watchlist'] = $count_bw['email']['watchlist'] + 1;
              }
            }
          }else{
            $result_bw['ae'] = 0;
      ...
      if ($result_feild['applnt_tel'] != '' ) {
        $text = $result_feild['applnt_tel'];
          if(strlen($result_feild['applnt_tel']) == 10) {
            $text = $result_feild['applnt_tel'];
        
            $str1 = substr($text, 0, 3);
            $str2 = substr($text, 3, 3);
            $str3 = substr($text, 6);
            $text1 = $str1.'-'.$str2.'-'.$str3;
          } else {
            $text = str_replace('-', '', $result_feild['applnt_tel']);
            $text1 = $result_feild['applnt_tel'];
          }
          if($text != '0000000000'){
            $sql = "SELECT * FROM `Case` WHERE case_status = 3  AND (complnt_contact_tel = '$text' OR complnt_contact_tel = '$text1') ";
          $query = $this->dbConn->query($sql);
          if ($query->num_rows > 0) {
            $result_bw['at'] = array();
            while ($result = $query->fetch_assoc()) {
              array_push($result_bw['at'], $result['reliable']);
              if ($result['reliable'] == 2) {
                array_push($blacklist_case,$result['case_id']);
                $count_bw['a_tel']['blacklist'] = $count_bw['a_tel']['blacklist'] + 1;
              }
              if( $result['reliable'] == 1){
                array_push($watchlist_case,$result['case_id']);
                $count_bw['a_tel']['watchlist'] = $count_bw['a_tel']['watchlist'] + 1;
              }
            }
          }else{
            $result_bw['at'] = 0;
      ...
      if($result_feild['applnt_type'] == 1){

        /* if ($result_feild['applntOrg_trade_number'] != '' && $result_feild['applntOrg_trade_number'] != '-') {
          $text = $result_feild['applntOrg_trade_number'];
          $sql = "SELECT * FROM `Case` WHERE case_status = 3  AND  REPLACE(complnt_trade_number, '-', '') = REPLACE('$text', '-', '') ";
          $query = $this->dbConn->query($sql);
          if ($query->num_rows > 0) {
            $result_bw['ctn'] = array();
            while ($result = $query->fetch_assoc()) {
              array_push($result_bw['ctn'], $result['reliable']);
              if ($result['reliable'] == 2) {
                array_push($blacklist_case,$result['case_id']);
                $count_bw['atnum']['blacklist'] = $count_bw['atnum']['blacklist'] + 1;
              }
              if( $result['reliable'] == 1){
                array_push($watchlist_case,$result['case_id']);
                $count_bw['atnum']['watchlist'] = $count_bw['atnum']['watchlist'] + 1;
              }
            }
          }else{
            $result_bw['ctn'] = 0;
        ...
        if ($result_feild['applntOrg_name'] != '' ) {
          $text = $result_feild['applntOrg_name'];
          $sql = "SELECT * FROM `Case` WHERE case_status = 3  AND complnt_name = '$text' ";
          $query = $this->dbConn->query($sql);
          if ($query->num_rows > 0) {
            $result_bw['cn_1'] = array();
            while ($result = $query->fetch_assoc()) {
              array_push($result_bw['cn_1'], $result['reliable']);
              if ($result['reliable'] == 2) {
                array_push($blacklist_case,$result['case_id']);
                $count_bw['aname']['blacklist'] = $count_bw['aname']['blacklist'] + 1;
              }
              if( $result['reliable'] == 1){
                array_push($watchlist_case,$result['case_id']);
                $count_bw['aname']['watchlist'] = $count_bw['aname']['watchlist'] + 1;
              }
            }
          }else{
            $result_bw['cn_1'] = 0;
        ...
        /* if ($result_feild['applntOrg_tel'] != '' ) {
          if(strlen($result_feild['applntOrg_tel']) == 10) {
            $text = $result_feild['applntOrg_tel'];
            ...
            $text = str_replace('-', '', $result_feild['applntOrg_tel']);
            $text1 = $result_feild['applntOrg_tel'];
          }
          if($text != '0000000000'){
              $sql = "SELECT * FROM `Case` WHERE case_status = 3  AND (complnt_contact_tel = '$text' OR complnt_contact_tel = '$text1') ";
            $query = $this->dbConn->query($sql);
            if ($query->num_rows > 0) {
              $result_bw['ctp'] = array();
              while ($result = $query->fetch_assoc()) {
                array_push($result_bw['ctp'], $result['reliable']);
                if ($result['reliable'] == 2) {
                  array_push($blacklist_case,$result['case_id']);
                  $count_bw['atel']['blacklist'] = $count_bw['atel']['blacklist'] + 1;
                }
                if( $result['reliable'] == 1){
                  array_push($watchlist_case,$result['case_id']);
                  $count_bw['atel']['watchlist'] = $count_bw['atel']['watchlist'] + 1;
                }
              }
            }else{
              $result_bw['ctp'] = 0;
        ...
        if ($result_feild['applnt_firstname'] != '' && $result_feild['applnt_lastname'] != '') {
          $firstname = $result_feild['applnt_firstname'];
          $lastname = $result_feild['applnt_lastname'];
          $text = $firstname.' '.$lastname;
          $sql = "SELECT * FROM `Case` WHERE case_status = 3  AND complnt_contact_name like '%" . $text . "'";
          $query = $this->dbConn->query($sql); 
          if ($query->num_rows > 0) {
            $result_bw['fl'] = array();
            while ($result = $query->fetch_assoc()) {
              array_push($result_bw['fl'], $result['reliable']);
              if ($result['reliable'] == 2) {
                array_push($blacklist_case,$result['case_id']);
                $count_bw['fl']['blacklist'] = $count_bw['fl']['blacklist'] + 1;
              }
              if( $result['reliable'] == 1){
                array_push($watchlist_case,$result['case_id']);
                $count_bw['fl']['watchlist'] = $count_bw['fl']['watchlist'] + 1;
              }
            }
          }else{
            $result_bw['fl'] = 0;
      ...
      $count['blacklist'] = count(array_values(array_unique($blacklist_case)));
      $count['watchlist'] = count(array_values(array_unique($watchlist_case)));
      
      $status_bw= $this->check_list_status_bw($result_bw);
      
      $status['count'] = $count;
      $status['status'] = $status_bw;
      
    }else if($formSetId == 2 ){

      if ($result_feild['complnt_name'] != '' ) {
        $text = $result_feild['complnt_name'];
        $sql = "SELECT * FROM `Case` WHERE case_status = 3  AND complnt_name = '$text' ";
        $query = $this->dbConn->query($sql);
        if ($query->num_rows > 0) {
          $result_bw['cn'] = array();
          while ($result = $query->fetch_assoc()) {
            array_push($result_bw['cn'], $result['reliable']);
            if ($result['reliable'] == 2) {
              array_push($blacklist_case,$result['case_id']);
              $count_bw['company']['blacklist'] = $count_bw['company']['blacklist'] + 1;
            }
            if( $result['reliable'] == 1){
              array_push($watchlist_case,$result['case_id']);
              $count_bw['company']['watchlist'] = $count_bw['company']['watchlist'] + 1;
            }
          }
        }else{
          $result_bw['cn'] = 0;
      ...
      /* if ($result_feild['complnt_trade_number'] != '' ) {
        $text = $result_feild['complnt_trade_number'];
        $sql = "SELECT * FROM `Case` WHERE case_status = 3  AND complnt_trade_number = '$text' ";
        $query = $this->dbConn->query($sql);
        if ($query->num_rows > 0) {
          $result_bw['ctn'] = array();
          while ($result = $query->fetch_assoc()) {
            ...
            if ($result['reliable'] == 2) {
              array_push($blacklist_case,$result['case_id']);
              $count_bw['number']['blacklist'] = $count_bw['number']['blacklist'] + 1;
            }
            if( $result['reliable'] == 1){
              array_push($watchlist_case,$result['case_id']);
              $count_bw['number']['watchlist'] = $count_bw['number']['watchlist'] + 1;
            }
          }
        }else{
          $result_bw['ctn'] = 0;
      ...
      if ($result_feild['complnt_contact_name'] != '' ) {
        $text = $result_feild['complnt_contact_name'];
        $sql = "SELECT * FROM `Case` WHERE case_status = 3  AND complnt_contact_name = '$text' ";
        $query = $this->dbConn->query($sql);
        if ($query->num_rows > 0) {
          $result_bw['ccn'] = array();
          while ($result = $query->fetch_assoc()) {
            array_push($result_bw['ccn'], $result['reliable']);
            if ($result['reliable'] == 2) {
              array_push($blacklist_case,$result['case_id']);
              $count_bw['name']['blacklist'] = $count_bw['name']['blacklist'] + 1;
            }
            if( $result['reliable'] == 1){
              array_push($watchlist_case,$result['case_id']);
              $count_bw['name']['watchlist'] = $count_bw['name']['watchlist'] + 1;
            }
          }
        }else{
          $result_bw['ccn'] = 0;
      ...
      if ($result_feild['complnt_contact_email'] != '' ) {
        
        $text = $result_feild['complnt_contact_email'];
        if($text != "no@no.com" && $text != "No@no.com" && $text != "No@No.com" && strtolower($text) != "none@none.com" ){
          $sql = "SELECT * FROM `Case` WHERE case_status = 3  AND complnt_contact_email = '$text' ";
          $query = $this->dbConn->query($sql);
          if ($query->num_rows > 0) {
      ...
      if ($result_feild['complnt_contact_tel'] != '' ) {
        if(strlen($result_feild['complnt_contact_tel']) == 10) {
          $text = $result_feild['complnt_contact_tel'];
          ...
          $text = str_replace('-', '', $result_feild['complnt_contact_tel']);
          $text1 = $result_feild['complnt_contact_tel'];
        }
        if($text != '0000000000'){
          $sql = "SELECT * FROM `Case` WHERE case_status = 3  AND (complnt_contact_tel = '$text' OR complnt_contact_tel = '$text1') ";
          $query = $this->dbConn->query($sql);
          if ($query->num_rows > 0) {
            ...
            while ($result = $query->fetch_assoc()) {
              ...
              if ($result['reliable'] == 2) {
                array_push($blacklist_case,$result['case_id']);
                $count_bw['tel']['blacklist'] = $count_bw['tel']['blacklist'] + 1;
              }
              if( $result['reliable'] == 1){
                array_push($watchlist_case,$result['case_id']);
                $count_bw['tel']['watchlist'] = $count_bw['tel']['watchlist'] + 1;
              }
            }
          }else{
            $result_bw['at'] = 0;
      ...
      /* $count['blacklist'] = max($count_bw['company']['blacklist'] ,$count_bw['number']['blacklist'] ,$count_bw['name']['blacklist'] , $count_bw['email']['blacklist'] , $count_bw['tel']['blacklist']);
      
      $count['watchlist'] = max($count_bw['company']['watchlist'] , $count_bw['number']['watchlist'] ,$count_bw['name']['watchlist'] , $count_bw['email']['watchlist'] , $count_bw['tel']['watchlist']); */
      /* $testshow =array_values(array_unique($watchlist_case)); */
      /* print_r($testshow); 
      exit(); */
      $status_bw = $this->check_list_status_bw($result_bw);
...
class case_open extends case_base
{
  ...
  {
    // if (in_array($_SESSION['admin']['empId'], array('1', '7'))) {
    //   echo '<br>compType_id.: '.$compType_id;
    //   echo '<br>compTypeSub1.: '.$compTypeSub1;
    //   echo '<br>compTypeSub2.: '.$compTypeSub2;
    // }
    ...
    // if (in_array($_SESSION['admin']['empId'], array('1', '7'))) {
    //   echo '<br>sql.: '.$sql;
    //   echo '<br>compType_name.: '.$arr_compType_name["compType_name"];
    //   echo '<br>compType_other_flag.: '.$arr_compType_name["compType_other_flag"];
    //   echo '<br>';
    // }
    ...
    // if (in_array($_SESSION['admin']['empId'], array('1', '7'))) {
    //   echo '<br>sql.: '.$sql;
    //   echo '<br>compTypeSub1_name.: '.$arr_compType_name["compTypeSub1_name"];
    //   echo '<br>';
    // }
    ...
    // if (in_array($_SESSION['admin']['empId'], array('1', '7'))) {
    //   echo '<br>sql.: '.$sql;
    //   echo '<br>compTypeSub2_name.: '.$arr_compType_name["compTypeSub2_name"];
    //   echo '<br>';
    // }
  ...
  public function check_blacklist_watchlist_openDetail($data,$formSetId)
  {
    $result_bw = array();
    $data_bw = array();
    /* print_r($data); */
    /* status_bw */
    if($formSetId == 1 || $formSetId == 2 || $formSetId == 9 ){
      $count_name = ['applnt_ident','a_fl','applnt_email','applnt_tel','applntOrg_trade_number','applntOrg_name','applntOrg_tel'];
      $count_bw = array();
      foreach ($count_name as $key => $value) {
        $count_bw[$value]['blacklist'] = 0;
        $count_bw[$value]['watchlist'] = 0;
      ...
      if ($data['applnt_ident'] != '' && $data['applnt_ident'] != '-') {
        $text = $data['applnt_ident'];
        $sql = "SELECT * FROM `Case` WHERE case_status = 3  AND REPLACE(complnt_trade_number, '-', '') = REPLACE('$text', '-', '') ";
        $query = $this->dbConn->query($sql);
        if ($query->num_rows > 0) {
          $result_bw['cnt'] = array();
          while ($result = $query->fetch_assoc()) {
            array_push($result_bw['cnt'], $result['reliable']);
            if ($result['reliable'] == 2) {
              $count_bw['applnt_ident']['blacklist'] = $count_bw['applnt_ident']['blacklist'] + 1;
            }
            if( $result['reliable'] == 1){
              $count_bw['applnt_ident']['watchlist'] = $count_bw['applnt_ident']['watchlist'] + 1;
            }
          }
        }else{
          $result_bw['cnt'] = 0;
      ...
      if ($data['applnt_firstname'] != '' && $data['applnt_lastname'] != '') {
        $firstname = $data['applnt_firstname'];
        $lastname = $data['applnt_lastname'];
        $text = $firstname.' '.$lastname;
        $sql = "SELECT * FROM `Case` WHERE case_status = 3  AND complnt_contact_name like '%" . $text . "'";
        $query = $this->dbConn->query($sql); 
        if ($query->num_rows > 0) {
          ...
          while ($result = $query->fetch_assoc()) {
            array_push($result_bw['fl'], $result['reliable']);
            if ($result['reliable'] == 2) {
              $count_bw['applnt_firstname']['blacklist'] = $count_bw['applnt_firstname']['blacklist'] + 1;
            }
            if( $result['reliable'] == 1){
              $count_bw['applnt_firstname']['watchlist'] = $count_bw['applnt_firstname']['watchlist'] + 1;
            }
          }
        }else{
          $result_bw['fl'] = 0;
      ...
      if ($data['applnt_email'] != '' ) {
        $text = $data['applnt_email'];
        if($text != "no@no.com" && $text != "No@no.com" && $text != "No@No.com" && strtolower($text) != "none@none.com"){
          ...
          if ($query->num_rows > 0) {
            ...
            while ($result = $query->fetch_assoc()) {
              array_push($result_bw['ae'], $result['reliable']);
              if ($result['reliable'] == 2) {
                $count_bw['applnt_email']['blacklist'] = $count_bw['applnt_email']['blacklist'] + 1;
              }
              if( $result['reliable'] == 1){
                $count_bw['applnt_email']['watchlist'] = $count_bw['applnt_email']['watchlist'] + 1;
              }
            }
          }else{
            $result_bw['ae'] = 0;
      ...
      if ($data['applnt_tel'] != '' ) {
        if(strlen($data['applnt_tel']) == 10) {
          $text = $data['applnt_tel'];
          ...
          $text = str_replace('-', '', $data['applnt_tel']);
          $text1 = $data['applnt_tel'];
        }
        ...
        if($text != '0000000000'){
          ...
          if ($query->num_rows > 0) {
            ...
            while ($result = $query->fetch_assoc()) {
              array_push($result_bw['at'], $result['reliable']);
              if ($result['reliable'] == 2) {
                $count_bw['applnt_tel']['blacklist'] = $count_bw['applnt_tel']['blacklist'] + 1;
              }
              if( $result['reliable'] == 1){
                $count_bw['applnt_tel']['watchlist'] = $count_bw['applnt_tel']['watchlist'] + 1;
              }
            }
          }else{
            $result_bw['at'] = 0;
      ...
      if($data['applnt_type'] == 1){

        if ($data['applntOrg_trade_number'] != '' && $data['applntOrg_trade_number'] != '-' ) {
          $text = $data['applntOrg_trade_number'];
          $sql = "SELECT * FROM `Case` WHERE case_status = 3  AND REPLACE(complnt_trade_number, '-', '') = REPLACE('$text', '-', '')  ";
          $query = $this->dbConn->query($sql);
          if ($query->num_rows > 0) {
            $result_bw['ctn'] = array();
            while ($result = $query->fetch_assoc()) {
              array_push($result_bw['ctn'], $result['reliable']);
              if ($result['reliable'] == 2) {
                $count_bw['applntOrg_trade_number']['blacklist'] = $count_bw['applntOrg_trade_number']['blacklist'] + 1;
              }
              if( $result['reliable'] == 1){
                $count_bw['applntOrg_trade_number']['watchlist'] = $count_bw['applntOrg_trade_number']['watchlist'] + 1;
              }
            }
          }else{
            $result_bw['ctn'] = 0;
        ...
        if ($data['applntOrg_name'] != '' ) {
          $text = $data['applntOrg_name'];
          $sql = "SELECT * FROM `Case` WHERE case_status = 3  AND complnt_name = '$text' ";
          $query = $this->dbConn->query($sql);
          if ($query->num_rows > 0) {
            $result_bw['cn'] = array();
            while ($result = $query->fetch_assoc()) {
              array_push($result_bw['cn'], $result['reliable']);
              if ($result['reliable'] == 2) {
                $count_bw['applntOrg_name']['blacklist'] = $count_bw['applntOrg_name']['blacklist'] + 1;
              }
              if( $result['reliable'] == 1){
                $count_bw['applntOrg_name']['watchlist'] = $count_bw['applntOrg_name']['watchlist'] + 1;
              }
            }
          }else{
            $result_bw['cn'] = 0;
        ...
        if ($data['applntOrg_tel'] != '' ) {
          if(strlen($data['applntOrg_tel']) == 10) {
            $text = $data['applntOrg_tel'];
            ...
            $text = str_replace('-', '', $data['applntOrg_tel']);
            $text1 = $data['applntOrg_tel'];
          }
          if($text != '0000000000'){
            $sql = "SELECT * FROM `Case` WHERE case_status = 3  AND (complnt_contact_tel = '$text' OR complnt_contact_tel = '$text1')";
            if ($query->num_rows > 0) {
              ...
              while ($result = $query->fetch_assoc()) {
                array_push($result_bw['ctp'], $result['reliable']);
                if ($result['reliable'] == 2) {
                  $count_bw['applntOrg_tel']['blacklist'] = $count_bw['applntOrg_tel']['blacklist'] + 1;
                }
                if( $result['reliable'] == 1){
                  $count_bw['applntOrg_tel']['watchlist'] = $count_bw['applntOrg_tel']['watchlist'] + 1;
                }
              }
            }else{
              $result_bw['ctp'] = 0;
    ...
    }else if($formSetId == 3 || $formSetId == 4 || $formSetId == 10 ){
      $count_name = ['number','company','name','tel','email'];
      $count_bw = array();
      foreach ($count_name as $key => $value) {
        $count_bw[$value]['blacklist'] = 0;
      ...
      if ($data['complnt_name'] != '' ) {
        $text = $data['complnt_name'];
        $sql = "SELECT * FROM `Case` WHERE case_status = 3  AND complnt_name = '$text' ";
        /* $sql = "SELECT `case_id` , IF(reliable!=0, reliable, cpr_reliable) AS reliable 
              FROM `Corporate` cp
              LEFT JOIN `Case` c on cp.cpr_companyname = c.complnt_name
              WHERE c.case_status = 3  AND cpr_comp_type = 2 AND complnt_name = '$text' "; */
        $query = $this->dbConn->query($sql);
        if ($query->num_rows > 0) {
          $result_bw['cn'] = array();
          while ($result = $query->fetch_assoc()) {
      ...
      if ($data['complnt_trade_number'] != '' ) {
        $text = $data['complnt_trade_number'];
        $sql = "SELECT * FROM `Case` WHERE case_status = 3  AND complnt_trade_number = '$text' ";
        /* $sql = "SELECT `case_id` , IF(reliable!=0, reliable, cpr_reliable) AS reliable 
              FROM `Corporate` cp
              LEFT JOIN `Case` c on cp.cpr_companyname = c.complnt_name
              WHERE c.case_status = 3  AND cpr_comp_type = 2 AND complnt_trade_number = '$text' "; */
        $query = $this->dbConn->query($sql);
        if ($query->num_rows > 0) {
          $result_bw['ctn'] = array();
          while ($result = $query->fetch_assoc()) {
      ...
      if ($data['complnt_contact_name'] != '' ) {
        $text = $data['complnt_contact_name'];
        $sql = "SELECT * FROM `Case` WHERE case_status = 3  AND complnt_contact_name = '$text' ";
        /* $sql = "SELECT `case_id` , IF(reliable!=0, reliable, cpr_reliable) AS reliable 
              FROM `Corporate` cp
              LEFT JOIN `Case` c on cp.cpr_companyname = c.complnt_name
              WHERE c.case_status = 3  AND cpr_comp_type = 2 AND complnt_contact_name = '$text' "; */
        $query = $this->dbConn->query($sql);
        if ($query->num_rows > 0) {
          $result_bw['ccn'] = array();
          while ($result = $query->fetch_assoc()) {
      ...
      if ($data['complnt_contact_email'] != '' ) {
        $text = $data['complnt_contact_email'];
        if($text != "no@no.com" && $text != "No@no.com" && $text != "No@No.com" && strtolower($text) != "none@none.com"){
          $sql = "SELECT * FROM `Case` WHERE case_status = 3  AND complnt_contact_email = '$text' ";
          $query = $this->dbConn->query($sql);
          if ($query->num_rows > 0) {
      ...
      if ($data['complnt_contact_tel'] != '' ) {
        if(strlen($data['complnt_contact_tel']) == 10) {
          $text = $data['complnt_contact_tel'];
          ...
          $text = str_replace('-', '', $data['complnt_contact_tel']);
          $text1 = $data['complnt_contact_tel'];
        }
        if($text != '0000000000'){
          $sql = "SELECT * FROM `Case` WHERE case_status = 3  AND (complnt_contact_tel = '$text' OR complnt_contact_tel = '$text1') ";
          $query = $this->dbConn->query($sql);
  ...
  public function setFromList_openDetailCase($formSetId, $formSetName, $formSetNo)
  {
    ...
    $data_bw = $this->check_blacklist_watchlist_openDetail($rs_case["case_feild"],$formSetId);
  ...
  //-- ฟังก์ชั่นแก้ไขค่าประเภทเรื่องร้องเรียนจาก Case ID --//
  public function editFromSetForCompType_editcase()
  {
    $arr_formSetList = array();
    $status_response = "00";
    $setOther = '';
    if ($this->compType_other_flag == "1") {
      $setOther = ", compType_other = '$this->compType_other' ";
    }

    $sql = "UPDATE `Case`
            SET compType_id = '$this->compType_id'
            , compTypeSub1_id = '$this->compTypeSub1'
            , compTypeSub2_id = '$this->compTypeSub2' 
            $setOther
            WHERE `case_id`='$this->case_id' ";
    // if (in_array($_SESSION['admin']['empId'], array('1', '7'))) {
    //   echo '<br>sql.: '.$sql;
    ...
    $query = $this->dbConn->query($sql);
    if ($query) {
      $arr_formSetList["status_response"] = '00';
    } else {
      $arr_formSetList["status_response"] = '01';
    }

    // $arr_formSetList["sql"] = $sql;
    ...
    foreach ($type_contact as $fId) {
      ...
      if ($post["applnt_ident_IdxFs_" . $fId] != '' || $post["applnt_firstname_IdxFs_" . $fId] != '' || $post["applnt_lastname_IdxFs_" . $fId] != '') {
        if ($post["applnt_ident_IdxFs_" . $fId] != '') {
          $applnt_ident = preg_replace('/[^0-9]/', '', $post["applnt_ident_IdxFs_" . $fId]);
          $sql_contact_chk = "SELECT * FROM `Contact_thai`
                                  WHERE ct_card='" . $applnt_ident . "'
        ...
        if ($count_contact_chk == 0) {
          $sql_contact = "INSERT
                              ...
                              VALUES(
                                ...
                                '" . $post["applnt_import_export_IdxFs_" . $fId] . "',
          ...
          if ($idx_fId == 0 && ($post["applnt_chkType_IdxFs_" . $fId] == 2)) {
      ...
      if ($idx_fId == 0) {
        ...
        $post["applntOrg_email_IdxFs_" . $fId] = $post["applnt_email_IdxFs_" . $fId];

        if ($post["compTypeSub1_id"] == 2) {
          $post["applntOrg_country_id_IdxFs_" . $fId] = '162';
        }
      } else if ($idx_fId == 1) {
        ...
        $post["applntOrg_address_IdxFs_" . $fId] = $post["complnt_contact_address_IdxFs_" . $fId];
        ...
        $post["applnt_valid_dbd_IdxFs_" . $fId] = $post["complnt_valid_dbd" . $fId];
        ...
        $post["applntOrg_web_IdxFs_" . $fId] = $post["complnt_web_IdxFs_" . $fId];
      }
        ...
        if ($count_contact_chk == 0) {
          $sql_contact_org = "INSERT
                                ...
                                VALUES(
                                  ...
                                  '" . $post["applntOrg_web_IdxFs_" . $fId] . "',
                                  '" . $post["applnt_email_IdxFs_" . $fId] . "',
                                  '" . $post["applntOrg_address_IdxFs_" . $fId] . "',
                                  '" . $post["applntOrg_prov_id_IdxFs_" . $fId] . "',
                                  '" . $post["applntOrg_zipcode_IdxFs_" . $fId] . "',
                                  '" . $post["applnt_valid_dbd_IdxFs_" . $fId] . "',
                                  '" . $post["applntOrg_country_id_IdxFs_" . $fId] . "',
                                  '" . $post["applntOrg_contact_name_IdxFs_" . $fId] . "',
                                  '0',
                                  NOW(),
                                  '" . $this->admin_id . "',
                                  '0'
          ...
          $sql_contact_org = "UPDATE
                              ...
                              cpr_web = '" . $post["applntOrg_web_IdxFs_" . $fId] . "',
  ...
  {
    /* print_r($file['caseAttach_file']['size']);
    exit(); */
    ...
    if ($post["caseCh_id"] == "") {
      ...
      $field_focus = "caseCh_id";
    }



    if ($post["complnt_contact_tel_IdxFs_" . $fId_b] == "") {
      $status_response = "02";
      $status_response_text = "กรุณาป้อนหมายเลขโทรศัพท์ติดต่อของผู้ถูกร้องเรียน";
      $field_focus = "complnt_contact_tel_IdxFs_" . $fId_b;
    }

    if ($post["compTypeSub1_id"] == 2) {
      if ($post["complnt_mobile_code"] == '') {
        $status_response = "02";
        $status_response_text = "กรุณาเลือกรหัสประเทศของผู้ถูกร้องเรียน";
        $field_focus = "complnt_contact_tel_IdxFs_" . $fId_b;
      }
    } else {
      if ($post["applnt_type_IdxFs_" . $fId_a] == 2) {
        if ($post["applnt_mobile_code"] == '') {
          $status_response = "02";
          $status_response_text = "กรุณาเลือกรหัสประเทศของผู้ร้องเรียน";
          $field_focus = "applnt_mobile_IdxFs_" . $fId_a;
        }
      } else {
        if ($post["applntOrg_mobile_code"] == '') {
          $status_response = "02";
          $status_response_text = "กรุณาเลือกรหัสประเทศของผู้ร้องเรียน";
          $field_focus = "applntOrg_tel_IdxFs_" . $fId_a;
        }
      }
    }

    if ($post["complnt_contact_email_IdxFs_" . $fId_b] == "") {
      $status_response = "02";
      $status_response_text = "กรุณาป้อน E-mail ที่ติดต่อของผู้ถูกร้องเรียน";
      $field_focus = "complnt_contact_email_IdxFs_" . $fId_b;
    }
    if ($post["applnt_chkType_IdxFs_" . $fId_a] == '') {
      $status_response = "02";
      $status_response_text = "กรุณากรอกข้อมูลบริษัท หรือเลือกยื่นในนามบุคคลธรรมดา";
      $field_focus = "applnt_chkType_IdxFs_" . $fId_a;
    }
    ...
    if (!isset($post["applnt_status_IdxFs_" . $fId_a])) {
      ...
      if ($post["applnt_email_IdxFs_" . $fId_a] == "") {
        echo 1;
        $status_response = "02";
        $status_response_text = "กรุณาป้อน E-Mail ของผู้ร้องเรียน";
        $field_focus = "applnt_email_IdxFs_" . $fId_a;
      }

      if ($post["applnt_mobile_IdxFs_" . $fId_a] == "") {
        $status_response = "02";
        $status_response_text = "กรุณาป้อนเบอร์โทรศัพท์มือถือของผู้ร้องเรียน";
        $field_focus = "applnt_mobile_IdxFs_" . $fId_a;
      }
    }
    ...
    while ($rs_field = $qr_field->fetch_assoc()) {
      foreach ($post as $key => $value) {
        ...
        if ($rs_field["fieldset_name"] == $keySplit[0]) {
          ...
          } else if ($rs_field["fieldset_type"] == "5") {
            $cct = substr_count($post[$key], "_");
            if ($cct > 0) {
              $status_response = "02";
              if ($status_response_text == "success") {
                $desTextAlert = $rs_field["fieldset_description"];
                $status_response_text = "กรุณากรอก " . $desTextAlert . " ให้ครบ 10 หลัก!";
                $field_focus = $key;
              }
            }
    ...
    if ($status_response == "00") {
      ...
      if ($qr_ins_case) {
        ...
        while ($rs_field = $qr_field->fetch_assoc()) {
          // if (in_array($_SESSION['admin']['empId'], array('1', '7'))) {  
          //   echo '<br>.:---------------------';
          //   echo '<br>.:  fieldset_name:'.$rs_field["fieldset_name"];
          // }
          foreach ($post as $key => $value) {
            $keySplit = explode('_IdxFs_', $key);
            // if (in_array($_SESSION['admin']['empId'], array('1', '7'))) {  
            //   echo '<br>.:  - key: '.$key.' - '.$keySplit[0];
            // }
            if ($rs_field["fieldset_name"] == $keySplit[0]) {
  ...
  public function update_case($post, $file)
  {
    ...
    if ($status_response == "00") {
      //-- บันทึก ข้อมูล Case --//
      // $office_id_update = '';
      // if(!empty($post["office_type_IdxFs_5"])){
      //   $office_id_update = " , `office_id`='".$post["office_type_IdxFs_5"]."' ";
      // }
      ...
      $sql_upd_case = "UPDATE
                        `Case` SET
                          ...
                          `office_id`='" . $post["office_type_IdxFs_5"] . "',
      ...
      if ($qr_upd_case) {
        ...
        // if($post["applnt_chkType_IdxFs_".$fId_a] == 2){
        //   $sqltest = "UPDATE `Field_Values` SET `fieldset_value` = '' WHERE `case_id` = '$last_case_id' AND `fieldset_id` = 68";
        // }
  ...
  public function open_case_check_blacklist_watchlist($data)
  {

    $result_bw = array();
    $data_bw = array();
    $count_name = ['ai_number','a_fl','a_email','am_tel','a_number','a_company','a_tel','number','company','name','tel','email'];
    $count_bw = array();
    foreach ($count_name as $key => $value) {
      $count_bw[$value]['blacklist'] = 0;
      $count_bw[$value]['watchlist'] = 0;
    ...
    /* 1 2 9 == ผู้ร้องเรียน */
    /* 3 4 10 == ผู้ถูกร้องเรียน */
    if($data['frmset_id'][0] == 1 || $data['frmset_id'][0] == 2 || $data['frmset_id'][0] == 9){
      $applnt_ident = "applnt_ident_IdxFs_".$data['frmset_id'][0];
      $applnt_firstname = "applnt_firstname_IdxFs_".$data['frmset_id'][0];
      $applnt_lastname = "applnt_lastname_IdxFs_".$data['frmset_id'][0];
      $applnt_email = "applnt_email_IdxFs_".$data['frmset_id'][0];
      $applnt_mobile ="applnt_mobile_IdxFs_".$data['frmset_id'][0];
      $applntOrg_trade_number ="applntOrg_trade_number_IdxFs_".$data['frmset_id'][0];
      $applntOrg_name ="applntOrg_name_IdxFs_".$data['frmset_id'][0];
      $applntOrg_tel ="applntOrg_tel_IdxFs_".$data['frmset_id'][0];
      ...
      if ($data[$applnt_ident] != '' && $data[$applnt_ident] != '-') {
        ...
        $query = $this->dbConn->query($sql);
        if ($query->num_rows > 0) {
          $result_bw[$applnt_ident] = array();
          while ($result = $query->fetch_assoc()) {
            array_push($result_bw[$applnt_ident], $result['reliable']);
            if ($result['reliable'] == 2) {
              $count_bw['ai_number']['blacklist'] = $count_bw['ai_number']['blacklist'] + 1;
            }
            if( $result['reliable'] == 1){
              $count_bw['ai_number']['watchlist'] = $count_bw['ai_number']['watchlist'] + 1;
            }
          }
        }else{
          $result_bw[$applnt_ident] = 0;
        }
      }
      ...
      if ($data[$applnt_firstname] != '' && $data[$applnt_lastname] != '') {
        ...
        $query = $this->dbConn->query($sql); 
        if ($query->num_rows > 0) {
          $result_bw[$applnt_firstname] = array();
          while ($result = $query->fetch_assoc()) {
            array_push($result_bw[$applnt_firstname], $result['reliable']);
            if ($result['reliable'] == 2) {
              $count_bw['a_fl']['blacklist'] = $count_bw['a_fl']['blacklist'] + 1;
            }
            if( $result['reliable'] == 1){
              $count_bw['a_fl']['watchlist'] = $count_bw['a_fl']['watchlist'] + 1;
            }
          }
        }else{
          $result_bw[$applnt_firstname] = 0;
        }
      }
      ...
      if ($data[$applnt_email] != '' ) {
        ...
        if($text != "no@no.com" && $text != "No@no.com" && $text != "No@No.com" && strtolower($text) != "none@none.com"){
          ...
          $query = $this->dbConn->query($sql);
          if ($query->num_rows > 0) {
            $result_bw[$applnt_email] = array();
            while ($result = $query->fetch_assoc()) {
              array_push($result_bw[$applnt_email], $result['reliable']);
              if ($result['reliable'] == 2) {
                $count_bw['a_email']['blacklist'] = $count_bw['a_email']['blacklist'] + 1;
              }
              if( $result['reliable'] == 1){
                $count_bw['a_email']['watchlist'] = $count_bw['a_email']['watchlist'] + 1;
              }
            }
          }else{
            $result_bw[$applnt_email] = 0;
          }
        }else{
          $result_bw[$applnt_email] = 0;
        }
      ...
      if ($data[$applnt_mobile] != '' ) {
       
        if(strlen($data[$applnt_mobile]) == 10) {
          $text = $data[$applnt_mobile];
          ...
          $text = str_replace('-', '', $data[$applnt_mobile]);
          $text1 = $data[$applnt_mobile];
        }
        ...
        if($text != '0000000000'){
          ...
          $query = $this->dbConn->query($sql);
          if ($query->num_rows > 0) {
            $result_bw[$applnt_mobile] = array();
            while ($result = $query->fetch_assoc()) {
              array_push($result_bw[$applnt_mobile], $result['reliable']);
              if ($result['reliable'] == 2) {
                $count_bw['am_tel']['blacklist'] = $count_bw['am_tel']['blacklist'] + 1;
              }
              if( $result['reliable'] == 1){
                $count_bw['am_tel']['watchlist'] = $count_bw['am_tel']['watchlist'] + 1;
              }
            }
          }else{
            $result_bw[$applnt_mobile] = 0;
          }
        }else{
          $result_bw[$applnt_mobile] = 0;
        }
      ...
      if ($data[$applntOrg_trade_number] != '' && $data[$applntOrg_trade_number] != '' ) {
        ...
        $query = $this->dbConn->query($sql);
        if ($query->num_rows > 0) {
          $result_bw[$applntOrg_trade_number] = array();
          while ($result = $query->fetch_assoc()) {
            array_push($result_bw[$applntOrg_trade_number], $result['reliable']);
            if ($result['reliable'] == 2) {
              $count_bw['a_number']['blacklist'] = $count_bw['a_number']['blacklist'] + 1;
            }
            if( $result['reliable'] == 1){
              $count_bw['a_number']['watchlist'] = $count_bw['a_number']['watchlist'] + 1;
            }
          }
        }else{
          $result_bw[$applntOrg_trade_number] = 0;
        }
      }
      ...
      if ($data[$applntOrg_name] != '' ) {
        ...
        $query = $this->dbConn->query($sql);
        if ($query->num_rows > 0) {
          $result_bw[$applntOrg_name] = array();
          while ($result = $query->fetch_assoc()) {
            array_push($result_bw[$applntOrg_name], $result['reliable']);
            if ($result['reliable'] == 2) {
              $count_bw['a_company']['blacklist'] = $count_bw['a_company']['blacklist'] + 1;
            }
            if( $result['reliable'] == 1){
              $count_bw['a_company']['watchlist'] = $count_bw['a_company']['watchlist'] + 1;
            }
          }
        }else{
          $result_bw[$applntOrg_name] = 0;
        }
      }
      ...
      if ($data[$applntOrg_tel] != '' ) {
        ...
        if($text != '0000000000'){
          ...
          $query = $this->dbConn->query($sql);
          if ($query->num_rows > 0) {
            $result_bw[$applntOrg_tel] = array();
            while ($result = $query->fetch_assoc()) {
              array_push($result_bw[$applntOrg_tel], $result['reliable']);
              if ($result['reliable'] == 2) {
                $count_bw['a_tel']['blacklist'] = $count_bw['a_tel']['blacklist'] + 1;
              }
              if( $result['reliable'] == 1){
            ...
            $result_bw[$applntOrg_tel] = 0;
          }
        }else{
          $result_bw[$applntOrg_tel] = 0;
        }
      }
      if(in_array(1,$result_bw[$applnt_ident]) && in_array(2,$result_bw[$applnt_ident])){
        $data_bw[$applnt_ident] = 3;
      }else if(!in_array(1,$result_bw[$applnt_ident]) && in_array(2,$result_bw[$applnt_ident])){
        $data_bw[$applnt_ident] = 2;
      }else if(in_array(1,$result_bw[$applnt_ident]) && !in_array(2,$result_bw[$applnt_ident])){
        $data_bw[$applnt_ident] = 1;
      }else{
        $data_bw[$applnt_ident] = 0;
      }
      if(in_array(1,$result_bw[$applnt_firstname]) && in_array(2,$result_bw[$applnt_firstname])){
        $data_bw[$applnt_firstname] = 3;
      }else if(!in_array(1,$result_bw[$applnt_firstname]) && in_array(2,$result_bw[$applnt_firstname])){
        $data_bw[$applnt_firstname] = 2;
      }else if(in_array(1,$result_bw[$applnt_firstname]) && !in_array(2,$result_bw[$applnt_firstname])){
        $data_bw[$applnt_firstname] = 1;
      }else{
        $data_bw[$applnt_firstname] = 0;
      }

      if(in_array(1,$result_bw[$applnt_email]) && in_array(2,$result_bw[$applnt_email])){
        $data_bw[$applnt_email] = 3;
      }else if(!in_array(1,$result_bw[$applnt_email]) && in_array(2,$result_bw[$applnt_email])){
        $data_bw[$applnt_email] = 2;
      }else if(in_array(1,$result_bw[$applnt_email]) && !in_array(2,$result_bw[$applnt_email])){
        $data_bw[$applnt_email] = 1;
      }else{
        $data_bw[$applnt_email] = 0;
      }

      if(in_array(1,$result_bw[$applnt_mobile]) && in_array(2,$result_bw[$applnt_mobile])){
        $data_bw[$applnt_mobile] = 3;
      }else if(!in_array(1,$result_bw[$applnt_mobile]) && in_array(2,$result_bw[$applnt_mobile])){
        $data_bw[$applnt_mobile] = 2;
      }else if(in_array(1,$result_bw[$applnt_mobile]) && !in_array(2,$result_bw[$applnt_mobile])){
        $data_bw[$applnt_mobile] = 1;
      }else{
        $data_bw[$applnt_mobile] = 0;
      }

      if(in_array(1,$result_bw[$applntOrg_trade_number]) && in_array(2,$result_bw[$applntOrg_trade_number])){
        $data_bw[$applntOrg_trade_number] = 3;
      }else if(!in_array(1,$result_bw[$applntOrg_trade_number]) && in_array(2,$result_bw[$applntOrg_trade_number])){
        $data_bw[$applntOrg_trade_number] = 2;
      }else if(in_array(1,$result_bw[$applntOrg_trade_number]) && !in_array(2,$result_bw[$applntOrg_trade_number])){
        $data_bw[$applntOrg_trade_number] = 1;
      }else{
        $data_bw[$applntOrg_trade_number] = 0;
      }

      if(in_array(1,$result_bw[$applntOrg_name]) && in_array(2,$result_bw[$applntOrg_name])){
        $data_bw[$applntOrg_name] = 3;
      }else if(!in_array(1,$result_bw[$applntOrg_name]) && in_array(2,$result_bw[$applntOrg_name])){
        $data_bw[$applntOrg_name] = 2;
      }else if(in_array(1,$result_bw[$applntOrg_name]) && !in_array(2,$result_bw[$applntOrg_name])){
        $data_bw[$applntOrg_name] = 1;
      }else{
        $data_bw[$applntOrg_name] = 0;
      }

      if(in_array(1,$result_bw[$applntOrg_tel]) && in_array(2,$result_bw[$applntOrg_tel])){
        $data_bw[$applntOrg_tel] = 3;
      }else if(!in_array(1,$result_bw[$applntOrg_tel]) && in_array(2,$result_bw[$applntOrg_tel])){
        $data_bw[$applntOrg_tel] = 2;
      }else if(in_array(1,$result_bw[$applntOrg_tel]) && !in_array(2,$result_bw[$applntOrg_tel])){
        $data_bw[$applntOrg_tel] = 1;
      }else{
        $data_bw[$applntOrg_tel] = 0;
      }
    ...
    /* if($data['frmset_id'][0] == 1 || $data['frmset_id'][0] == 2 || $data['frmset_id'][0] == 9){
      ...
      $applntOrg_name ="applntOrg_name_IdxFs_".$data['frmset_id'][0];
      $applntOrg_tel ="applntOrg_tel_IdxFs_".$data['frmset_id'][0];
      $applnt_chkType ="applnt_chkType_IdxFs_".$data['frmset_id'][0];
      
      // เช็ค blacklist เลขบัตรประชาชน 
      if($data[$applnt_ident] !=''){
        $text = $data[$applnt_ident];
        $sql = "SELECT `case_id` , IF(reliable!=0, reliable, cpr_reliable) AS reliable 
              FROM `Corporate` cp
              LEFT JOIN `Case` c on cp.cpr_companyname = c.complnt_name
              WHERE c.case_status = 3  AND cpr_comp_type = 2 AND cpr_numbertrade = '$text'";
        $query = $this->dbConn->query($sql);
        if ($query->num_rows > 0) {
          $result_bw[$applnt_ident] = array();
          while ($result = $query->fetch_assoc()) {
      ...
      // เช็ค blacklist ชื่อ นามสกุล 
      if($data[$applnt_firstname] !='' && $data[$applnt_lastname] !=''){
        $fn = $data[$applnt_firstname];
        $ln = $data[$applnt_lastname];
        $sql = "SELECT `case_id` , IF(reliable!=0, reliable, cpr_reliable) AS reliable 
              FROM `Corporate` cp
              LEFT JOIN `Case` c on cp.cpr_companyname = c.complnt_name
              WHERE c.case_status = 3  AND cpr_comp_type = 2 AND cpr_contactfname = '$fn' AND cpr_contactlname = '$ln' ";
        $query = $this->dbConn->query($sql);
        if ($query->num_rows > 0) {
          $result_bw[$applnt_firstname] = array();
          while ($result = $query->fetch_assoc()) {
      ...
      // เช็ค blacklist E-Mail 
      if($data[$applnt_email] != ''){
        $text = $data[$applnt_email];
        $sql = "SELECT `case_id` , IF(reliable!=0, reliable, cpr_reliable) AS reliable 
              FROM `Corporate` cp
              LEFT JOIN `Case` c on cp.cpr_companyname = c.complnt_name
              WHERE c.case_status = 3  AND cpr_comp_type = 2 AND cpr_email = '$text' ";
        $query = $this->dbConn->query($sql);
        if ($query->num_rows > 0) {
          $result_bw[$applnt_email] = array();
          while ($result = $query->fetch_assoc()) {
      ...
      // เช็ค blacklist เบอร์โทรศัพท์มือถือ 
      if($data[$applnt_mobile] != ''){
        $text = str_replace('-', '', $data[$applnt_mobile]);
        $sql = "SELECT `case_id` , IF(reliable!=0, reliable, cpr_reliable) AS reliable 
              FROM `Corporate` cp
              LEFT JOIN `Case` c on cp.cpr_companyname = c.complnt_name
              WHERE c.case_status = 3  AND cpr_comp_type = 2 AND cpr_telephone = '$text' ";
        $query = $this->dbConn->query($sql);
        if ($query->num_rows > 0) {
          $result_bw[$applnt_mobile] = array();
          while ($result = $query->fetch_assoc()) {
      ...
      //เช็ค ว่าเป็นตัวแทนมั้ย 
      if($data[$applnt_chkType] == 1){
         //เช็ค blacklist เลขนิติบุคคล 
        if($data[$applntOrg_trade_number] != ''){
          $text = $data[$applntOrg_trade_number];
          $sql = "SELECT `case_id` , IF(reliable!=0, reliable, cpr_reliable) AS reliable 
                FROM `Corporate` cp
                LEFT JOIN `Case` c on cp.cpr_companyname = c.complnt_name
        ...
        // เช็ค blacklist เบอร์โทรศัพท์ บริษัท 
        if($data[$applntOrg_tel] != ''){
          $text = str_replace('-', '', $data[$applntOrg_tel]);
          $sql = "SELECT `case_id` , IF(reliable!=0, reliable, cpr_reliable) AS reliable 
                FROM `Corporate` cp
    ...
    /* sql 2 ตัวล่างจะเป็น add email tel เข้า db */
    /* $sql_email ="SELECT * FROM `Field_Values` a LEFT JOIN `Field_Set` b ON (a.fieldset_id=b.fieldset_id) WHERE b.fieldset_id IN (39,88,186) AND fieldset_value != ''";
    $query_email = $this->dbConn->query($sql_email);
    if ($query_email->num_rows > 0) {
      while ($result_email = $query_email->fetch_assoc()) {
        $caseId = $result_email['case_id'];
        $fieldsetValue = $result_email['fieldset_value'];
        $sql_upd_case = "UPDATE `Case` SET `complnt_contact_email` = '$fieldsetValue' WHERE `case_id` = '$caseId'";
        
        $email = $this->dbConn->query($sql_upd_case);
      }
    } */
    /* $sql_tel ="SELECT * FROM `Field_Values` a LEFT JOIN `Field_Set` b ON (a.fieldset_id=b.fieldset_id) WHERE b.fieldset_id IN (38,87,185) AND fieldset_value != ''";
    $query_tel = $this->dbConn->query($sql_tel);
    if ($query_tel->num_rows > 0) {
      while ($result_tel = $query_tel->fetch_assoc()) {
        $caseId = $result_tel['case_id'];
        $fieldsetValue = $result_tel['fieldset_value'];
        $sql_upd_case = "UPDATE `Case` SET `complnt_contact_tel` = '$fieldsetValue' WHERE `case_id` = '$caseId'";
        
        $tel = $this->dbConn->query($sql_upd_case);
      }
    }
    exit(); */
    
    
    if($data['frmset_id'][1] == 3 || $data['frmset_id'][1] == 4 || $data['frmset_id'][1] == 10){
      
      $complnt_trade_number = "complnt_trade_number_IdxFs_".$data['frmset_id'][1];
      $complnt_name = "complnt_name_IdxFs_".$data['frmset_id'][1];
      $complnt_contact_name = "complnt_contact_name_IdxFs_".$data['frmset_id'][1];
      $complnt_contact_tel = "complnt_contact_tel_IdxFs_".$data['frmset_id'][1];
      $complnt_contact_email = "complnt_contact_email_IdxFs_".$data['frmset_id'][1];
      ...
      if ($data[$complnt_trade_number] != '' ) {
       ...
       /*  $sql = "SELECT `case_id` , IF(reliable!=0, reliable, cpr_reliable) AS reliable 
        ...
        $query = $this->dbConn->query($sql);
        if ($query->num_rows > 0) {
          $result_bw[$complnt_trade_number] = array();
          while ($result = $query->fetch_assoc()) {
            array_push($result_bw[$complnt_trade_number], $result['reliable']);
            if ($result['reliable'] == 2) {
              $count_bw['number']['blacklist'] = $count_bw['number']['blacklist'] + 1;
            }
            if( $result['reliable'] == 1){
          ...
          $result_bw[$complnt_trade_number] = 0;
        }
      }
      ...
      if ($data[$complnt_name] != '' ) {
       ...
       /*  $sql = "SELECT `case_id` , IF(reliable!=0, reliable, cpr_reliable) AS reliable 
        ...
        $query = $this->dbConn->query($sql);
        if ($query->num_rows > 0) {
          $result_bw[$complnt_name] = array();
          while ($result = $query->fetch_assoc()) {
            array_push($result_bw[$complnt_name], $result['reliable']);
            if ($result['reliable'] == 2) {
              $count_bw['company']['blacklist'] = $count_bw['company']['blacklist'] + 1;
            }
            if( $result['reliable'] == 1){
      ...
      if ($data[$complnt_contact_name] != '' ) {
        ...
        $query = $this->dbConn->query($sql);
        if ($query->num_rows > 0) {
          $result_bw[$complnt_contact_name] = array();
          while ($result = $query->fetch_assoc()) {
            array_push($result_bw[$complnt_contact_name], $result['reliable']);
            if ($result['reliable'] == 2) {
              $count_bw['name']['blacklist'] = $count_bw['name']['blacklist'] + 1;
            }
            if( $result['reliable'] == 1){
          ...
          $result_bw[$complnt_contact_name] = 0;
        }
      }
      ...
      if ($data[$complnt_contact_email] != '' ) {
        ...
        if($text != "no@no.com" && $text != "No@no.com" && $text != "No@No.com" && strtolower($text) != "none@none.com"){
          ...
          $query = $this->dbConn->query($sql);
          if ($query->num_rows > 0) {
            $result_bw[$complnt_contact_email] = array();
            while ($result = $query->fetch_assoc()) {
              array_push($result_bw[$complnt_contact_email], $result['reliable']);
              if ($result['reliable'] == 2) {
                $count_bw['email']['blacklist'] = $count_bw['email']['blacklist'] + 1;
              }
              if( $result['reliable'] == 1){
            ...
            $result_bw[$complnt_contact_email] = 0;
          }
        }else{
          $result_bw[$complnt_contact_email] = 0;
        }
      ...
      if ($data[$complnt_contact_tel] != '' ) {
        ...
        if($text != '0000000000'){
          ...
          $query = $this->dbConn->query($sql);
          if ($query->num_rows > 0) {
            $result_bw[$complnt_contact_tel] = array();
            while ($result = $query->fetch_assoc()) {
              array_push($result_bw[$complnt_contact_tel], $result['reliable']);
              if ($result['reliable'] == 2) {
                $count_bw['tel']['blacklist'] = $count_bw['tel']['blacklist'] + 1;
              }
              if( $result['reliable'] == 1){
            ...
            $result_bw[$complnt_contact_tel] = 0;
          }
        }else{
          $result_bw[$complnt_contact_tel] = 0;
        }
      ...
      if(in_array(1,$result_bw[$complnt_trade_number]) && in_array(2,$result_bw[$complnt_trade_number])){
        $data_bw[$complnt_trade_number] = 3;
      }else if(!in_array(1,$result_bw[$complnt_trade_number]) && in_array(2,$result_bw[$complnt_trade_number])){
        $data_bw[$complnt_trade_number] = 2;
      }else if(in_array(1,$result_bw[$complnt_trade_number]) && !in_array(2,$result_bw[$complnt_trade_number])){
        $data_bw[$complnt_trade_number] = 1;
      }else{
        $data_bw[$complnt_trade_number] = 0;
      }
      if(in_array(1,$result_bw[$complnt_name]) && in_array(2,$result_bw[$complnt_name])){
        $data_bw[$complnt_name] = 3;
      }else if(!in_array(1,$result_bw[$complnt_name]) && in_array(2,$result_bw[$complnt_name])){
        $data_bw[$complnt_name] = 2;
      }else if(in_array(1,$result_bw[$complnt_name]) && !in_array(2,$result_bw[$complnt_name])){
        $data_bw[$complnt_name] = 1;
      }else{
        $data_bw[$complnt_name] = 0;
      }
      if(in_array(1,$result_bw[$complnt_contact_name]) && in_array(2,$result_bw[$complnt_contact_name])){
        $data_bw[$complnt_contact_name] = 3;
      }else if(!in_array(1,$result_bw[$complnt_contact_name]) && in_array(2,$result_bw[$complnt_contact_name])){
        $data_bw[$complnt_contact_name] = 2;
      }else if(in_array(1,$result_bw[$complnt_contact_name]) && !in_array(2,$result_bw[$complnt_contact_name])){
        $data_bw[$complnt_contact_name] = 1;
      }else{
        $data_bw[$complnt_contact_name] = 0;
      }
      if(in_array(1,$result_bw[$complnt_contact_tel]) && in_array(2,$result_bw[$complnt_contact_tel])){
        $data_bw[$complnt_contact_tel] = 3;
      }else if(!in_array(1,$result_bw[$complnt_contact_tel]) && in_array(2,$result_bw[$complnt_contact_tel])){
        $data_bw[$complnt_contact_tel] = 2;
      }else if(in_array(1,$result_bw[$complnt_contact_tel]) && !in_array(2,$result_bw[$complnt_contact_tel])){
        $data_bw[$complnt_contact_tel] = 1;
      }else{
        $data_bw[$complnt_contact_tel] = 0;
      }
      if(in_array(1,$result_bw[$complnt_contact_email]) && in_array(2,$result_bw[$complnt_contact_email])){
        $data_bw[$complnt_contact_email] = 3;
      }else if(!in_array(1,$result_bw[$complnt_contact_email]) && in_array(2,$result_bw[$complnt_contact_email])){
        $data_bw[$complnt_contact_email] = 2;
      }else if(in_array(1,$result_bw[$complnt_contact_email]) && !in_array(2,$result_bw[$complnt_contact_email])){
        $data_bw[$complnt_contact_email] = 1;
      }else{
        $data_bw[$complnt_contact_email] = 0;
      }
      
    }
    $data_bw['count'] = $count_bw;
    $data_bw['frmset_a'] = $data['frmset_id'][0];
    $data_bw['frmset_b'] = $data['frmset_id'][1];
...
class case_detail extends case_open
{
  ...
  public function check_blacklist_watchlist($data,$formSetId)
  {
    $result_bw = array();
    $data_bw = array();
    /* print_r($data); */
    /* status_bw */
    if($formSetId == 1 || $formSetId == 2 || $formSetId == 9 ){
      $count_name = ['applnt_ident','applnt_firstname','applnt_email','applnt_tel','applntOrg_trade_number','applntOrg_name','applntOrg_tel'];
      $count_bw = array();
      foreach ($count_name as $key => $value) {
        $count_bw[$value]['blacklist'] = 0;
        $count_bw[$value]['watchlist'] = 0;
  ...
  //--f ฟังก์ชั่นกำหนดรูปแบบฟอร์ม แบบต่างๆรอไว้ --//
  public function setFromList_detailCase($formSetId, $formSetName, $formSetNo)
  {
    ...
    //  echo "</pre>";
    //   exit();
    $data_bw = $this->check_blacklist_watchlist($rs_case["case_feild"],$formSetId);
    /* print_r('<pre>');
    print_r($data_bw ); */
    if (count($this->case_country == 0)) {
      $this->case_country = $this->countryList();
  ...
  public function close_process($post)
  {
    ...
    // if($post['caseClose_reliable'] == ''){
    //   $status_response = "02";
    //   $status_response_text = "ขออภัย...กรุณาเลือกความน่าเชื่อถือ !";
    ...
    if ($post['reliable'] == 2 && $post['reliable_sub'] == '') {
      $status_response = "02";
      $status_response_text = "ขออภัย...กรุณาเลือก Watchlist หรือ Blacklist !";
    ...
    if ($status_response == "00") {
      ...
      if ($process_type_id == 1 || $process_type_id == 2) {
        if ($process_type_id == 1) {
          ...
          $sql_udp_case_type .= "  `reliable` = '" . $post['reliable'] . "', ";
          $sql_udp_case_type .= "  `reliable_sub` = '" . $post['reliable_sub'] . "' ";
        } else if ($process_type_id == 2) {
  ...
  public function close_case($post)
  {
    ...
    if ($caseClose_id == "" || $case_close_resultProcess == "" || $post["caseClose_reliable"] == "" || $post["caseClose_sendEmail"] == "") {
      $status_response = "02";
      $status_response_text = "กรุณาเลือกสถานะการยุติข้อร้องเรียน ผลการดำเนินงานและตรวจสอบสถานะให้ครบถ้วน";
    } else {
      $sql_udp_case = "UPDATE `Case`
                       SET
                         `case_status` = '3',
                         ...
                         `reliable` = '" . $post["caseClose_reliable"] . "'
                       WHERE `case_id` = '$case_id' ";
      $qr_udp_case = $this->dbConn->query($sql_udp_case);
      if (!$qr_udp_case) {
        $status_response = "01";
