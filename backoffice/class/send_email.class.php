...
class email extends main{
  ...
  public function send_email($post,$file){
   ...
   /*  $sql_case = "SELECT * FROM `Case` WHERE case_id = '".$_POST['case_id']."' ";
    ...
    if ($post['country_id'] == 162) {
      ...
      $to_ditp = ' DITP Service Center 1169 & DITP Care Team <br />
        The Department of International Trade Promotion (DITP) <br />
        Tel. (+66) 2507 - 8247 <br />';
    }
  ...
  public function send_email_close_case($post, $case_id, $member_id,$country_id = 162) {
    ...
    if ($country_id != 162) {
      ...
      <div class="content" style="width:auto; height:auto; padding:20px; font-family: Tahoma, Verdana, Segoe, sans-serif; font-size:16px;">
        ...
        Dear '.$post["applnt_firstname"].' '.$post["applnt_lastname"].' <br /> <br />
        &nbsp;&nbsp; Regarding case numbe ('.$case_id.') with DITP Care, we hereby inform you that your case has been closed. <br /><br />
      

        This is an automated notification. Please do not reply to this email. If you have any questions, please contact us at the phone number provided below.  <br /> <br />
      </div>
      ...
      <div class="content" style="width:auto; height:auto; padding:20px; font-family: Tahoma, Verdana, Segoe, sans-serif; font-size:16px;">
        <!-- ข้อความ -->
        เรียน คุณ '.$post["applnt_firstname"].' '.$post["applnt_lastname"].' <br /> <br />
        &nbsp;&nbsp; ตามที่ท่านได้ยื่นเรื่องร้องเรียนในระบบบริการตรวจสอบคู่ค้าเบื้องต้น และ ให้คำปรึกษาด้านข้อพิพาททางการค้าระหว่างประเทศ DITP Care หมายเลขเคส ('.$case_id.') นั้น <br /><br />
        ...
        บัดนี้ เคสของท่านได้รับการยุติเรียบร้อยแล้ว <br /><br />

        อีเมลฉบับนี้เป็นการแจ้งข้อมูลจากระบบอัตโนมัติ กรุณาอย่าตอบกลับ หากท่านมีข้อสงสัยหรือต้องการสอบถามรายละเอียดเพิ่มเติม สามารถติดต่อตามเบอร์โทรศัพท์ด้านล่าง  <br /> <br />
      </div>
    ...
    if ($country_id != 162) {
      $subject = 'Results of International Trade Complaint and Dispute Management (DITP CARE)';
    }else{
