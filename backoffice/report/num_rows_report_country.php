...
function report_issue(){
  ...
  if($respon == ""){
    ...
    $sql .= " WHERE 1  AND c.applnt_country_id <> 0 AND ct.compType_section = '".$_SESSION['admin']['empSection']."' $where_caseCh $whereSearch $where_date $where_comp $where_prod $where_caseCh
    ...
    LEFT JOIN `Incorrect_Type` AS it ON c.incType_id = it.incType_id";
    $sql .= " WHERE 1 AND AND c.applnt_country_id <> 0 ct.compType_section = '".$_SESSION['admin']['empSection']."' AND a.caseAsign_status = 0 $where_caseCh $whereSearch $where_date $where_comp
...
//   echo $sql;
  ...
  if($respon == ""){
    ...
    LEFT JOIN `Case_Channel` AS ch ON c.caseCh_id = ch.caseCh_id
    LEFT JOIN `Incorrect_Type` AS it ON c.incType_id = it.incType_id";
    $sql_caseCh .= " WHERE 1  AND c.applnt_country_id <> 0 AND ct.compType_section = '".$_SESSION['admin']['empSection']."' $where_caseCh $whereincase $whereSearch $where_date $where_comp $where_prod
         ...
         if($i == "0"){
           ...
           if($_SESSION['admin']['empSection'] == "1"){
             if($re['caseClose_id'] == "1"){
               ...
               $status = "Close กรมไม่สามารถดำเนินการได้ ";
               $closereject++;
