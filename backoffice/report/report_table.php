...
function report_issue($post)
{
  ...
  if ($post->member_comp_type_ditp == "") {
    $wheremembeditp = "";
  } else {
    $wheremembeditp = " AND c.complnt_valid_ditp = $post->member_comp_type_ditp";
    /* if ($post->member_comp_type_ditp == 1) {
      $wheremembeditp = " AND (complnt_valid_ditp IN (1, 2) AND complnt_valid_ditp_org != '')";
    } else if ($post->member_comp_type_ditp == 2) {
      $wheremembeditp = " AND c.complnt_valid_ditp = $post->member_comp_type_ditp AND c.complnt_valid_ditp_org == '' ";
    } else {
      $wheremembeditp = " AND c.complnt_valid_ditp = $post->member_comp_type_ditp";
    } */
  }
  ...
  while ($re = $query_edit_pass->fetch_assoc()) {
    ...
    if ($post->status_complaint == "2") {
      ...
      //  $reliable = $re['reliable'];

      $caseCh_col_arr["reliable"] = '<span class="txt_nol">' . $reliable . '</span>';
      $caseCh_col_arr["case_id"] = '<span class="txt_nol">' . $re['case_id'] . '</span>';
      $caseCh_col_arr["case_receivedoc_date"] = '<div class="date_report"><span class="txt_nol">' . $date_receivedoc_ex . '</span></div>';
      $caseCh_col_arr["close"] = '<div class="date_report"><span class="txt_nol">' . $date_close_ex . '</span></div>';
      if ($re['caseDtl_title'] == "") {
        $caseDtl_title = "-";
      } else {
      ...
      if ($re['complnt_valid_ditp'] == "1") {
        $member_comp_type_ditp = "เป็นสมาชิกกรม";
      } elseif ($re['complnt_valid_ditp'] == "2") {
        if ($re['complnt_valid_ditp_org'] != "") {
      ...
      /* print_r($re['complnt_valid_ditp']);
      exit(); */
      $caseCh_col_arr["member_comp_type"] = '<div class="member_comp_report"><span class="txt_nol">' . $member_comp_type . '</span></div>';
      $caseCh_col_arr["member_comp_type_ditp"] = '<div class="member_comp_report"><span class="txt_nol">' . $member_comp_type_ditp . '</span></div>';
      ...
      while ($rs = $query_ass->fetch_assoc()) {
        $caseCh_col_arr["officename"] = '<div class="name_report"><span class="txt_nol">' . $rs['office_name'] . '</span></div><br>';
      }
      //  }else {
      ...
      while ($rs = $query_ass->fetch_assoc()) {
        ...
        $caseCh_col_arr["counsel"] = '<div class="name_report"><span class="txt_nol">' . $rs['emp_firstname'] . ' ' . $rs['emp_lastname'] . '</span></div><br>';
      }
      //  }
      // $caseCh_col_arr["officename"] = $counsel;
      // $caseCh_col_arr["counsel"] = '<div class="name_report"><span class="txt_nol">'.$rs['emp_firstname'].' '.$rs['emp_lastname'].'</span></div><br>';
      $caseCh_col_arr["complnt_trade_number"] = '<div class="name_report"><span class="txt_nol">' . $re['complnt_trade_number'] . '</span></div>';
      ...
      $arrd = array(
        '1'   => 'Close(overdue) : คู่กรณีสามารถตกลงกันได้', '2'  => 'Close(overdue) : คู่กรณีดำเนินการในส่วนที่เกี่ยวข้องต่อไป', '3'  => 'Close(overdue) : ไม่สามารถดำเนินการได้', '4'  => 'Close(overdue) : ไม่พบมูลความผิด', '5'  => 'Close(overdue) : ภาคทัณฑ์', '6'  => 'Close(overdue) : ตัดเงินเดือน', '7'  => 'Close(overdue) : ลดขั้นเงินเดือน', '8'  => 'Close(overdue) : ปลดออก', '9'  => 'Close(overdue) : ไล่ออก', '10' => 'Close(overdue) : อื่นๆ', '11' => 'Close(overdue) : ตรวจสอบความน่าเชื่อถือของบริษัท'
      );
