...
for ($row = 1; $row <= $highestRow; ++$row) {
	...
	if ($row == 1) {
		if( $dataRow[$row]['A'] != "Passport No."
				...
				|| $dataRow[$row]['K'] != "ประเทศ") {
			//
			if ($dataRow[$row]['A'] != "Passport No."){ $errorcolumn .="A,"; }
			if ($dataRow[$row]['B'] != "ชื่อ*") { $errorcolumn .="B,"; }
			if ($dataRow[$row]['C'] != "นามสกุล*") { $errorcolumn .="C,"; }
			if ($dataRow[$row]['D'] != "เลขนิติบุคคล") { $errorcolumn .="D,"; }
			if ($dataRow[$row]['E'] != "ประเภทธุรกิจ*") { $errorcolumn .="E,"; }
			if ($dataRow[$row]['F'] != "ตำแหน่ง") { $errorcolumn .="F,"; }
			if ($dataRow[$row]['G'] != "เบอร์โทรศัพท์*") { $errorcolumn .="G,"; }
			...
			if ($dataRow[$row]['K'] != "ประเทศ") { $errorcolumn .="K,"; }
...
for ($i=0;$i<$rowarr;$i++) {
	...
	$sql_card= "select name,id from Country where name = '".$namedDataArray[$i][10]."' AND country_status = '0' ";
  ...
  if ($namedDataArray[$i][1]!="" && $namedDataArray[$i][2]!="" && $namedDataArray[$i][4]!="" && $namedDataArray[$i][6]!="" && $namedDataArray[$i][8]!="" && $namedDataArray[$i][9]!="" ) {
		...
		// $sql_card= "select ct_id,ct_card from Contact_thai where ct_card = '".$namedDataArray[$i][0]."' AND ct_status = 0 AND ct_type = '2' AND ct_section='$emp_section' ";
		...
		if($namedDataArray[$i][0]!=''){
			...
			$sql_card= "select ct_id,ct_card from Contact_thai
										...
										AND ct_status = 0 AND ct_type = '2' AND ct_section='$emp_section' ";
		}
		$query_card = $conn->query($sql_card);
		if ($query_card->num_rows>0) {
      while($result_card = $query_card->fetch_assoc())
      {
        ...
        $sql_edit = "UPDATE Contact_thai SET
			...
			$sql_add = "INSERT INTO Contact_thai (ct_section,ct_type,ct_card,ct_firstname,ct_lastname,ct_numbertrade,ct_business_type,ct_career
																						,ct_homephone,ct_cellphone,ct_email,ct_address,Country_id,ct_import,ct_create_datetime,ct_createBy_id)
									VALUES ('$emp_section'
													,'2'
													,'".$namedDataArray[$i][0]."'
													,'".$namedDataArray[$i][1]."'
