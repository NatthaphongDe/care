...
if (isset($_REQUEST["method"]) && $_REQUEST["method"] == "save_case") {
	...
	$total_fileAttach = count($_FILES['caseAttach_file']['name']);
	if ($total_fileAttach > 0) {
		foreach ($_FILES['caseAttach_file']['size'] as $key => $val_size) {
			//print_r($val_size);
			if ($val_size == 0 || $val_size > 10485760) {
				//$status_response = "02";
				if ($val_size == 0) {
					$status_response_text = "การอัพโหลดเอกสารประกอบเรื่องร้องเรียนผิดพลาด";
				} else {
					$status_response_text = "ไม่สามารถอัพโหลดไฟล์เอกสารที่ขนาดเกิน 10 MB ได้";
				}
				/* echo json_encode(array('status_response' => $status_response, 'status_response_text' => $status_response_text));
				exit(); */
			}
		}
	}
...
if (isset($_REQUEST["method"]) && $_REQUEST["method"] == "save_open_case") {
	...
	// bizportal
	if ($res['applnt_type'] == 1) {
		$applnt_ident = $res['applntOrg_trade_number'];
	} else {
		$applnt_ident = $res['applnt_ident'];
	}
	if ($applnt_ident != '') {
		/* $token_biz = send_biz($applnt_ident);
		$ReferenceId = generateReferenceId();
		$type = ($_POST['applnt_type'] == 1 ? 'Juristic' : 'Citizen');
		$token = $token_biz->Result;
		$data = [
			"IdentityId" => "$applnt_ident",
			"IdentityType" => $type, // "Juristic" หรือ "Citizen" / "Foreigner"
			"ApplicationId" => "922",
			"Data" => [
				"Name" => "care",
				"Url" => "https://care.ditp.go.th/frontend/index.php?page=home&lang=1",
				"ReferenceId" => "$ReferenceId",
				"ReferenceNumber" =>  $case_id
			]
		];

		$app_request_import = app_request_import($data, $token);
		$sql_log = "INSERT INTO `log_bizprotal` (`input`,`output`,`case_id`,`status`,`type`) VALUES ('" . json_encode($data) . "','" . json_encode($app_request_import) . "','" . $case_id . "',1,1)";
		$conn->query($sql_log);

		$sql_update_req = "UPDATE `Case` SET `request_id` = '" . $app_request_import->data->requestId . "',`reference_id` ='" . $ReferenceId . "' WHERE `case_id` = '" . $case_id . "'";
		$conn->query($sql_update_req);
		$data_update = [
			"IdentityId" => "$applnt_ident",
			"IdentityType" => $type, // หรือ "Citizen" / "Foreigner"
			"ApplicationId" => "922",
			"RequestId" => $app_request_import->data->requestId,
			"Status" => "CHECK",
		...
		$app_request_update = app_request_update($data_update, $token);
		$sql_log_update = "INSERT INTO `log_bizprotal` (`input`,`output`,`case_id`,`status`,`type`) VALUES ('" . json_encode($data_update) . "','" . json_encode($app_request_update) . "','" . $case_id . "',1,2)";
...
if (isset($_REQUEST["method"]) && $_REQUEST["method"] == "assign_case") {
	...
	// bizportal
	if ($res['request_id'] != null && $applnt_ident != '') {
		/*$token_biz = send_biz($applnt_ident);
		$type = ($res['applnt_type'] == 1 ? 'Juristic' : 'Citizen');
		$token = $token_biz->Result;
		...
		$data_update = [
			...
			"Status" => "PENDING",
			"StatusOther" => "WAITING_AGENT_WORKING",
			"Remark" => "",
			"Option" => [
				"ExternalButtonOption" => [
					"Name" => "",
		 ...
		 $app_request_update = app_request_update($data_update, $token);
		$sql_log_update = "INSERT INTO `log_bizprotal` (`input`,`output`,`case_id`,`status`,`type`) VALUES ('" . json_encode($data_update) . "','" . json_encode($app_request_update) . "','" . $case_id . "',2,2)";
		$conn->query($sql_log_update); */

		//print_r($sql_log_update );
	}
	mysqli_close($conn);
	// bizportal end
	if ($response["status_response"] == "00") {
...
if (isset($_REQUEST["method"]) && $_REQUEST["method"] == "close_case") {
	...
	/* if ($res['request_id'] != null && $applnt_ident != '') {
		...
		$app_request_update = app_request_update($data_update, $token);
		$sql_log_update = "INSERT INTO `log_bizprotal` (`input`,`output`,`case_id`,`status`,`type`) VALUES ('" . json_encode($data_update) . "','" . json_encode($app_request_update) . "','" . $case_id . "',3,2)";
		$conn->query($sql_log_update);
	} */
	// bizportal end

	if ($response["status_response"] == "00") {
		if ($_POST["caseClose_sendEmail"] == 0) {
			$send_mail = $email_cls->send_email_close_case($rs_case["case_feild"], $_POST["case_id"], $rs_case['case']["case_createBy_id"]);
		} else {
			$send_mail["status_response"] = "00";
		}
...
if (isset($_REQUEST["method"]) && $_REQUEST["method"] == "edit_close_case") {
	include("../config/config.php");
	$case_close_resultProcess = $_POST['case_close_resultProcess'];
	$caseClose_reliable  = $_POST['caseClose_reliable'];
	$case_id = $_POST['case_id'];

	$sql_case = "UPDATE `Case` SET case_close_resultProcess = '" . $case_close_resultProcess . "',reliable = '" . $caseClose_reliable . "' WHERE case_id = '" . $case_id . "' ";
	$query_case = $conn->query($sql_case);

	if ($query_case) {
	?>
		<script>
			parent.iziToast_func.success('แก้ไข ผลการดำเนินงาน เรียบร้อยแล้ว', function() {
				window.parent.location.href = "index.php?page=case_detail&caseId=<?php echo $case_id ?>";
			});
		</script>
	<?php
	} else {
	?>
		<script>
			parent.iziToast_func.alert('เกิดข้อผิดพลาด กรุณาลองใหม่อีกครั้ง', function() {
				$(".bootbox", window.parent.document).on("hidden.bs.modal", function() {
					$('body', window.parent.document).addClass('modal-open');
				});
			});
...
if (isset($_REQUEST["method"]) && $_REQUEST["method"] == "send_email") {
	...
	$response =   array(
		'status_response' => $sendMail["status_response"],
...
function send_biz($AgentID)
{
	$curl = curl_init();
	$consumer_key = 'cba5e312-73a4-42aa-8a64-bd32aa794ff0';
	//$AgentID = '0125550046368';
	curl_setopt_array($curl, array(
		CURLOPT_URL => "https://api.egov.go.th/ws/auth/validate?ConsumerSecret=XglArSHOFdR&AgentID=" . $AgentID,
...
function app_request_import($data, $token)
{
	$curl = curl_init();
	$consumer_key = 'cba5e312-73a4-42aa-8a64-bd32aa794ff0';
	//$token ='';
	curl_setopt_array($curl, [
		CURLOPT_URL => "https://api.egov.go.th/ws/dga/bizportal/uat/applicationrequest/import",
		CURLOPT_RETURNTRANSFER => true,
		CURLOPT_ENCODING => "",
		CURLOPT_MAXREDIRS => 10,
		CURLOPT_TIMEOUT => 30,
		...
		CURLOPT_POSTFIELDS => json_encode($data), // ต้องใช้ json_encode()
		CURLOPT_HTTPHEADER => [
			"Consumer-Key: $consumer_key", // ต้องมีช่องว่างหลัง `:`
			"Content-Type: application/json",
			"Token: $token"
		],
	]);
...
function app_request_update($data, $token)
{
	$curl = curl_init();
	$consumer_key = 'cba5e312-73a4-42aa-8a64-bd32aa794ff0';
	//$token ='';
	curl_setopt_array($curl, [
		CURLOPT_URL => "https://api.egov.go.th/ws/dga/bizportal/uat/applicationrequest/update",
		CURLOPT_RETURNTRANSFER => true,
		CURLOPT_ENCODING => "",
		CURLOPT_MAXREDIRS => 10,
		CURLOPT_TIMEOUT => 30,
...
function generateReferenceId()
{
	...
	$year = date('Y');       // ปี (เช่น 2024)
	$monthDay = date('md');  // เดือนวัน (เช่น 0403 -> 3 เม.ย.)
	$hourMin = date('Hi');   // ชั่วโมงนาที (เช่น 1530 -> 15:30 น.)
	$randomPart1 = bin2hex(openssl_random_pseudo_bytes(4));
	$randomPart2 = bin2hex(openssl_random_pseudo_bytes(6));

	$ReferenceId = sprintf('%s-%s-%s-%s-%s', $randomPart1, $year, $monthDay, $hourMin, $randomPart2);
