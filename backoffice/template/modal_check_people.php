...
<body>
<form name="frm-checkPeople" method="post" action="?method=backToOpenCase" >
      ...
      <div class="modal-dialog">
          <div class="modal-content">
              <div class="modal-header">
                  ...
                  <h4 class="modal-title">ตรวจสอบข้อมูลจากทะเบียนราษฎร์</h4>
              </div>
              <div class="modal-body">
                <div class="row">
                  <div class="col-md-12">
                    ...
                    <div class="col-sm-11">
                        <input type="text" class="form-control input-mask" name="ident_number" value="<?php echo $_GET["textId"] ?>" data-inputmask="&apos;mask&apos;:&apos;9-99999-9999-999&apos;"  />
                    </div>
                    <div class="col-sm-1">
              ...
              <div class="modal-footer">
                  <button type="button" class="btn btn-success btn-open-model-confirm" onclick="send_people_status('1','<?php echo $_GET["frmSet"] ?>')" >ยืนยันข้อมูล</button>
      ...
      <div class="modal-dialog">
          <div class="modal-content">
              ...
              <div class="modal-body">
                <div class="row">
                  <div class="col-md-12">
                    <textarea name="note" id="note_people_check" rows="3" class="form-control textarea-no-resize"></textarea>
                  </div>
                </div>
              </div>
              <div class="modal-footer">
                  <button type="button" class="btn btn-success btn-success-final" onclick="">ยืนยัน</button>
              </div>
          </div>
      </div>
  </div>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $('.btn-open-model-invalid').click(function(){
      ...
      $('.btn-success-final').attr("onClick","send_people_status('2','<?php echo $_GET["frmSet"] ?>')");
    });
  });
  ...
  function send_people_status(status,frmset){
    if(status=="1"){
      if(confirm("กรุณายืนยันผลตรวจสอบ \"ข้อมูลถูกต้อง\" ")){
        $(window.opener.document).find("#check_people_logo_IdxFs_"+frmset).attr("src","img/btn_check_people_1.png");
        $(window.opener.document).find("#applnt_ident_valid_IdxFs_"+frmset).val(1);
        $(window.opener.document).find("#check_people_note_IdxFs_"+frmset).val($("#note_people_check").val());
        closeWindow();
      }
    }else if(status=="2"){
      if(confirm("กรุณายืนยันผลตรวจสอบ \"ข้อมูลไม่ถูกต้อง\" ")){
        $(window.opener.document).find("#check_people_logo_IdxFs_"+frmset).attr("src","img/btn_check_people_2.png");
        $(window.opener.document).find("#applnt_ident_valid_IdxFs_"+frmset).val(2);
        $(window.opener.document).find("#check_people_note_IdxFs_"+frmset).val($("#note_people_check").val());
        closeWindow();
      }
    }
