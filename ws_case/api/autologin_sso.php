   ...
   include ("../config/config.php");
    //    header('Content-Type: application/json');
        ...
        if($ck != 0){
            ...
            $sql = "INSERT INTO Member (member_fname,member_lname,member_cid,member_email,member_phone,member_type,member_status,member_condition,member_creDate,ssoid,member_address,member_postcode) VALUES ('".$post['member']['name']."','','".$post['naturalId']."','".$post['sub_member']['email']."','".$post['sub_member']['tel']."','".$post['type']."',1,1,NOW(),'".$post['ssoid']."','".$post['address']['address']." ".$post['address']['subdistrict']." ".$post['address']['district']." ".$post['address']['province']." ".$post['address']['postcode']."','".$post['address']['postcode']."') ";
            ...
            if ($conn->query($sql) === TRUE) {
                ...
                if($post['type'] == 1 || $post['type'] == 2){
                    ...
                    $sql_com = "INSERT INTO Member_comp (member_id,member_comp_name,member_comp_branch,member_comp_taxid,member_comp_address,member_comp_postcode,member_comp_phone,member_comp_type) VALUES ('$last_id','".$post['member']['name']."','','".$post['naturalId']."','".$post['address']['address']." ".$post['address']['subdistrict']." ".$post['address']['district']." ".$post['address']['province']." ".$post['address']['postcode']."','".$post['address']['postcode']."','".$post['sub_member']['tel']."','0',) ";   
                    $conn->query($sql);
                }
                $_SESSION["member"]["member_id"] = $ck;
                $_SESSION["member"]["member_type"] = $post['type'];
