...
$app->post('/register', function() use ($app) {
            ...
            array_push($arrayverify,'email', 'password');
            ...
            if ($res == USER_CREATED_SUCCESSFULLY) {
              ...
              if ($result != NULL) {
                ...
                $response["res_text"] = "อีเมล์นี้มีอยู่ในระบบแล้ว";
...
$app->post('/login', function() use ($app) {
            ...
            if ($db->checkLogin($email, $password)) {
              if($password == 'facebook'){
                ...
                $response['res_code'] = "02";
                $response['res_text'] = 'อีเมล์หรือรหัสผ่านไม่ถูกต้อง กรุณาลองอีกครั้ง';
            }
...
$app->get('/knowledge', function() use ($app) {
            ...
            if ($result != NULL) {
                ...
                while ($res = $result->fetch_assoc()) {
                    ...
                    $tmp["knowledge_incType_name"] = $res["incType_name"];
                    $tmp["knowledge_incType_name_en"] = $res["incType_name_en"];
                    array_push($response["res_result"], $tmp);
                }
                echoRespnse(200, $response);
            } else {
          ...
          $sort = $app->request->get('sort');
            ...
            if ($result != NULL) {
                ...
                foreach ($result as $key=>$value) { // code mock
                    ...
                    $tmp["message_sender"] = $res["sender_type"];
                    ...
                    $tmp['message_read'] = $res["msgBox_read_status"];
                    array_push($response["res_result"], $tmp);
                }
                echoRespnse(200, $response);
            } else{
...
$app->put('/user', 'authenticate', function($user_id) use($app) {
            ...
            if ($result) {
                ...
                $result['member_comp_img'] = BASE_URL.'data/img_membercom/'.$result['member_comp_id'].'/'.$result['member_comp_img'];
                $response["res_code"] = "00";
                $response["res_text"] = "แก้ไขข้อมูลส่วนตัวสำเร็จ";
