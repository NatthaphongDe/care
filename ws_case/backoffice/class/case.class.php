    ...
    if ($post->text != "") {
      ...
      $sql_case_condition .= "
        AND ( c.case_id = '" . (int)$post->text . "'
              ...
              OR (
                (c.complnt_type!=0 AND c.applntOrg_trade_number like '%$post->text%')
                OR (c.complnt_type=0 AND c.applnt_ident like '%$post->text%')
             )
             OR c.complnt_trade_number like '%$post->text%'
  ...
  public function check_list_blacklist_watchlist($data,$formSetId){
    ...
    /* if($formSetId == 1 ){
      ...
      if ($result_feild['applnt_firstname'] != '' && $result_feild['applnt_lastname'] != '') {
        $fn = $result_feild['applnt_firstname'];
        $ln = $result_feild['applnt_lastname'];
        $sql = "SELECT `case_id` , IF(reliable!=0, reliable, cpr_reliable) AS reliable 
              FROM `Corporate` cp
              LEFT JOIN `Case` c on cp.cpr_companyname = c.complnt_name
        ...
        if ($query->num_rows > 0) {
          $result_bw['fln'] = array();
          while ($result = $query->fetch_assoc()) {
            array_push($result_bw['fln'], $result['reliable']);
          }
        }else{
          $result_bw['fln'] = 0;
        }
      ...
      if($result_feild['applnt_type'] == 1){
        ...
        if ($result_feild['applntOrg_trade_number'] != '' ) {
          $text = $result_feild['applntOrg_trade_number'];
          $sql = "SELECT `case_id` , IF(reliable!=0, reliable, cpr_reliable) AS reliable 
                FROM `Corporate` cp
                LEFT JOIN `Case` c on cp.cpr_companyname = c.complnt_name
                WHERE c.case_status = 3  AND cpr_comp_type = 2 AND applntOrg_trade_number = '$text' ";
          $query = $this->dbConn->query($sql);
          if ($query->num_rows > 0) {
            $result_bw['ctn'] = array();
            while ($result = $query->fetch_assoc()) {
      ...
      if ($result_feild['complnt_contact_email'] != '' ) {
        ...
        if($text != "no@no.com" && $text != "No@no.com" && $text != "No@No.com"){
...
class case_detail extends case_open
{
  ...
  public function check_blacklist_watchlist($data,$formSetId)
  {
    ...
    if($formSetId == 3 || $formSetId == 4 || $formSetId == 10 ){
  ...
  public function close_case($post)
  {
    ...
    if ($caseClose_id == "" || $case_close_resultProcess == "" || $post["caseClose_reliable"] == "") {
      $status_response = "02";
