    ...
    if(status=="1"){
      ...
      if(!(dtype == 'other' || dtype == '' || dtype == 'non member')) {
        if(confirm("กรุณายืนยันผลตรวจสอบ \"เป็นสมาชิกกรม\" ")){
          $(window.opener.document).find("#check_ditp_logo_IdxFs_"+frmset).attr("src","img/ditp/btn-ditp-"+$("#ditp_type").val()+".png");
        ...
        if(confirm("กรุณายืนยันผลตรวจสอบ \"ไม่เป็นสมาชิกกรม\" ")){
          $(window.opener.document).find("#check_ditp_logo_IdxFs_"+frmset).attr("src","img/btn_check_ditp_2.png");
