...
$month_kpi = $_POST['month_kpi'];
 ...
 if($search_kpi==0){
   ...
   if($mon != ''){
    ...
    if($year_kpi !='' ){
      $case_d = " AND case_create_datetime  like '%".$year_kpi."".$mon."%' ";
    }
   }
  //  if($year_kpi !='' ){
 ...
 if($_SESSION["admin"]["empSection"]!=0){
  $sql_empGroup = " AND emp_g.empGroup_section = '".$_SESSION["admin"]["empSection"]."' ";
}
...
$num_close_bar =  ($num_close *100) /$case;
  // echo "<br>";
$case_active = $new + $num_pedding + $num_overdue;
  if($case_active == "0"){
    continue;  
  }
  // echo '5678<br>';
