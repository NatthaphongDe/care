...
class DbHandler {
        ...
        if($result->num_rows > 0){
           while($res = $result->fetch_assoc()){
              ...
              ///////////// end 2 ///////////////
                ...
                // $sql ="SELECT * FROM `Product_Type` WHERE prodType_level = ".$i;
                // $stmt1 = $this->conn->prepare($sql);
                // $stmt1->execute();
                // $result1 = $stmt1->get_result();
                // if($result1->num_rows > 0){
                //   while($res1 = $result1->fetch_assoc()){
                //     array_push($response,array(
                //                                 'prodType_id' =>$res1['prodType_id'],
                //                                 'prodType_level'=>$res1['prodType_level'],
                //                                 'prodType_name'=>$res1['prodType_name'],
                //                                 'prodType_name_en'=>$res1['prodType_name_en'],
                //                                 'sql'=>$sql
                //                               ));
                //    $test = $res1['prodType_id'];
                //   }
                // }else{
                //   // break;
                // }
        ...
        if($result->num_rows > 0){
          ...
          while($res = $result->fetch_assoc())
            ...
            $response = array(
              ...
              "compType_other_flag" => $res['compType_other_flag'],
              "levelmenu" => 1
  					);
  					$output[]=$response;
  					$id[]= $res['compType_id'];
          ...
          foreach ($output as $key => $product) {
            ...
            $response = array();
            //if($checkS){
            if (is_array($output[$key]['compType_Sub1']) || is_object($output[$key]['compType_Sub1']))
            {
    ...
    $incType_other,$prodType_other) {
        ...
        $applnt_ident','$applntOrg_trade_number','$applnt_firstname','$applnt_type','$applnt_ident_valid','$applnt_status','$complnt_trade_number','
        $complnt_name','$complnt_backlist','$applnt_valid_dbd','$applnt_valid_ditp','$case_receivedoc_date','$case_createBy_id','$applnt_lastname','$applntOrg_name',
        '$applnt_country_id','$complnt_country_id','$case_create_datetime','$incType_id','$applntOrg_country_id', (select compType_duration from `Complaint_Type` where compType_id = '$compType_id'),'$compType_other',
         '$incType_other','$prodType_other','$res2[office_id]')");
        $result = $stmt->execute();
        //$stmt->close();
        $mailsent = array();
        $namesent = array();
        ...
        if ($result) {
          ...
          $result = $this->func->sendEmail("ditp.noreply@gmail.com","Noreply DITP",$mailsent,$namesent,$subject,$html_message,"","","2");
            ...
            $n = 0;
            if (is_array($complnt_file) || is_object($complnt_file))
            {
  ...
  $company_postcode,$company_country_id, $company_phone, $company_fax, $comp_img, $user_img) {
    ...
    $cpr_import,$cpr_create_datetime,$cpr_createBy_id,$cpr_ststus){
      ...
      cpr_zipcode,cpr_department,cpr_contactfname,
      cpr_contactlname,Country_id,cpr_contact_person,
      cpr_import,cpr_create_datetime,cpr_createBy_id,cpr_ststus) values(
  ...
  public function conf_reg($mail){
        ...
        if($result->num_rows > 0){
          ...
          $message = " <div class=\"wrapper\" style=\"width:860px;background: #f8f8f8;\">
                        <div class=\"header\" style=\"width:auto\">
                        <img src=\"".BASE_URL."img/header_email.png\" srtlw=\"width:100%; height:auto;\" />
                        <div>
