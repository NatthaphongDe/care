 ...
 * method - POST
 */
$app->post('/dashboardChart', function() use ($app) {

    $startDate = $app->request->post('startDate');
    $stopDate = $app->request->post('stopDate');

    $response = array();
    $db = new DbHandler();
    $result = $db->getDashboardChart($startDate, $stopDate);
    if ($result != NULL) {
        $response["res_code"] = "00";
        $response["res_text"] = "แสดงข้อมูลสำเร็จ";
        $response["res_result"] = $result;
 ...
 * method - POST
 */
$app->post('/dashboardCase', function() use ($app) {
    ...
    $response = array();
    $db = new DbHandler();
    $result = $db->getDashboardCase($startDate, $stopDate);
    if ($result != NULL) {
        $response["res_code"] = "00";
        $response["res_text"] = "แสดงข้อมูลสำเร็จ";
        $response["res_result"] = $result;
...
/*** dashboardCaseStatus ***
 * url - /dashboardCaseStatus
 * method - POST
 */
$app->post('/dashboardCaseStatus', function() use ($app) {

    $startDate = $app->request->post('startDate');
    $stopDate = $app->request->post('stopDate');
    $status_complaint = $app->request->post('status_complaint');
    $applnt_country = $app->request->post('applnt_country');
    $office_id = $app->request->post('office_id');

    $text = $app->request->post('text');

    $response = array();
    $db = new DbHandler();
    $result = $db->getDashboardCaseStatus($app->request->post(), $startDate, $stopDate, $status_complaint, $applnt_country, $office_id, $text);
    if ($result != NULL) {
        $response["res_code"] = "00";
        $response["res_text"] = "แสดงข้อมูลสำเร็จ";
        $response["res_result"] = $result;
...
$app->post('/checkformset_test', function() use ($app) {
    ...
    $response = array();
    $db = new DbHandler();
    $result = $db->getFormSet1($compType1, $compType2, $compType3);
    if ($result != NULL ) {
        $response["res_code"] = "00";
        $response['res_text'] = "แสดงข้อมูลสำเร็จ";
        $response["res_result"] = $result;
...
$app->get('/country', function() use ($app) {
    ...
    if ($result != NULL) {
        ...
        while ($res = $result->fetch_assoc()) {
            ...
            $tmp["country_name_th"] = $res["name_th"];
            array_push($response["res_result"], $tmp);
        }
        echoRespnse(200, $response);
    } else {
 ...
 * method - GET
 */
$app->post('/typeComplaint', function() use ($app) {

    $type = $app->request->post('type');

    $response = array();
    $db = new DbHandler();
    $result = $db->getTypeComplaint($type);
    if ($result != NULL) {
        $response["res_code"] = "00";
        $response["res_text"] = "แสดงข้อมูลสำเร็จ";
        $response["res_result"] = $result;
...
$app->get('/testtypeComplaint', function() use ($app) {

    $response = array();
    $db = new DbHandler();
    $result = $db->testgetTypeComplaint();
    if ($result != NULL) {
        $response["res_code"] = "00";
        $response["res_text"] = "แสดงข้อมูลสำเร็จ";
        $response["res_result"] = $result;
 ...
 * method - GET
 */
$app->get('/officeType', function() use ($app) {
    $response = array();
    $db = new DbHandler();
    $result = $db->getOfficeType();
    if ($result != NULL) {
        $response["res_code"] = "00";
        $response["res_text"] = "แสดงข้อมูลสำเร็จ";
        $response["res_result"] = $result;
...
$app->get('/changeProduct', function() use ($app) {

    $response = array();
    $db = new DbHandler();
    $result = $db->changeProduct();
    if ($result != NULL) {
        $response["res_code"] = "00";
        $response["res_text"] = "แสดงข้อมูลสำเร็จ";
        $response["res_result"] = $result;
...
$app->get('/updateProduct', function() use ($app) {
...
/*** CaseDetail ***
 * url - /caseDetail/:type/:id
 * method - GET
 */
$app->get('/caseDetail/:type/:id', function($type, $id) use ($app) {

    
    $response = array();
    $db = new DbHandler();
   
    $result = $db->getCaseDetail($id);
    $response["res_code"] = "00";
    $response["res_text"] = "แสดงข้อมูลสำเร็จ";
    $response["corporate_number"] = $id;
    $response["res_result"] = array();
    if($result != NULL) {
        if($type == 1) {
            while ($res = $result->fetch_assoc()) {
                $tmp["case_id"] = $res["case_id"];
                if($res["company_type"] == 1) {
                    $company_type = 'ผู้ร้องเรียน';
                } else {
                    $company_type = 'ผู้ถูกร้องเรียน';
                }
                $tmp["company_type"] = $company_type;
                $tmp["complaint_type"] = $res["compTypeSub2_name"];
                if($res["reliable"] == 1) {
                    $reliable = 'เฝ้าระวัง (Watchlist)';
                } else if($res["reliable"] == 2) {
                    $reliable = 'แบล็กลิสต์ (Blacklist)';
                } else {
                    $reliable = 'ไม่มีสถานะ';
                }
                $tmp["company_status"] = $reliable;
                $tmp["case_close_datetime"] = $res["case_close_datetime"];
...
/*** CaseCompanyStatus ***
 * url - /caseCompanyStatus/:id
 * method - GET
 */
$app->get('/caseCompanyStatus/:id', function($id) use ($app) {

    $response = array();
    $db = new DbHandler();

    if(strtolower($id) == 'blacklist') {
        $id = 2;
    } else if(strtolower($id) == 'watchlist') {
        $id = 1;
    } 
    
    $result = $db->getCompanyStatusDetail($id);
    if($result != NULL) {
        if($id == 1) {
            $reliable = 'เฝ้าระวัง (Watchlist)';
        } else if($id == 2) {
        ...
        $response["res_code"] = "00";
        $response["res_text"] = "แสดงข้อมูลสำเร็จ";
        $response["company_status"] = $reliable;
        $response["res_result"] = array();
        while ($res = $result->fetch_assoc()) {
            $tmp["case_id"] = $res["case_id"];
            $tmp["case_create_datetime"] = $res["case_create_datetime"];
            $tmp["case_close_datetime"] = $res["case_close_datetime"];
            $tmp["case_title"] = $res["caseDtl_title"];
            $tmp["corporate_number"] = $res["complnt_trade_number"];
            $tmp["company_name"] = $res["complnt_name"];
            $tmp["complnt_contact_email"] = ($res["complnt_contact_email"] == null?"-":$res["complnt_contact_email"]);
            $tmp["complnt_contact_tel"] = ($res["complnt_contact_tel"] == null?"-":$res["complnt_contact_tel"]);
            $tmp["product_name"] = $res["product_name"];
            if($res["reliable"] == 1) {
                $reliable = 'เฝ้าระวัง (Watchlist)';
            } else if($res["reliable"] == 2) {
                $reliable = 'แบล็กลิสต์ (Blacklist)';
...
$app->get('/complaint', 'authenticate', function() use ($app) {
    ...
    echo $user_id;exit(); 
    // die();
    $result = $db->getAllUserComplaint($user_id);
    if ($result != NULL) {
        $response["res_code"] = "00";
        $response["res_text"] = "แสดงข้อมูลสำเร็จ";
        $response["res_result"] = $result;
 ...
 * url /complaintForStaff
 * method POST
 */
$app->post('/complaintForStaff', function() use ($app) {
    ...
    $result = $db->getAllComplaintForStaff($app->request->post());
    if ($result != NULL) {
        $response["res_code"] = "00";
        $response["res_text"] = "แสดงข้อมูลสำเร็จ";
        $response["res_result"] = $result;
 ...
 * method GET
 */
$app->get('/complaint/:id', function($case_id) use ($app) {
 ...
 * method POST
 */
$app->post('/complaintByCaseId', function() use ($app) {

    $user_id = '';

    $type = htmlspecialchars(trim($app->request->post('type')), ENT_QUOTES);
    $case_id = htmlspecialchars(trim($app->request->post('case_id')), ENT_QUOTES);
    ...
    if($type != 1) {
        if($type == 0) {
            ...
            $response["res_code"] = "01";
            $response["res_text"] = "ไม่มีสิทธิ์การเข้าถึงข้อมูล";
            echoRespnse(200, $response);
            $app->stop();
        }
    ...
    $response = array();
    $db = new DbHandler();
    $result = $db->getComplaintByCaseId($user_id, $case_id, $type);
    if ($result != NULL) {
        $response["res_code"] = "00";
        $response["res_text"] = "แสดงข้อมูลสำเร็จ";
        $response["res_result"] = $result;
...
/*** แสดงข้อมูลของผู้ใช้เท่านั้น ***
 * url /memberDetail
 * method GET
 */
$app->get('/memberDetail', 'authenticate', function() use ($app) {
    global $user_id;
    $response = array();
    $db = new DbHandler();
...
$app->post('/complaint', 'authenticate', function() use ($app) {
    ...
    $case_createBy_id = $user_id;
    $complnt_file_title = $app->request->post('complnt_file_title');
    $complnt_file = $_FILES;
    $totalFiles = 0;
    $isStaff = trim($app->request->post('isStaff'));
    $case_createBy_staff_id = trim($app->request->post('case_createBy_staff_id'));
    ...
    $applnt_import_export = $app->request->post('applnt_import_export');
    $applnt_gender = $app->request->post('applnt_gender');
    $applnt_career = $app->request->post('applnt_career');
    $applnt_prov_id = $app->request->post('applnt_prov_id');
    $applnt_zipcode = $app->request->post('applnt_zipcode');
    $applnt_country_id = $app->request->post('applnt_country_id');
    $applnt_type = trim($app->request->post('applnt_type'));
    $applnt_mobile_code = trim($app->request->post('applnt_mobile_code'));
    $applnt_mobile_country = trim($app->request->post('applnt_mobile_country'));

    $applntOrg_trade_number = htmlspecialchars(trim($app->request->post('applntOrg_trade_number')), ENT_QUOTES);
    $applntOrg_name = trim($app->request->post('applntOrg_name'));
    $applntOrg_import_export = trim($app->request->post('applntOrg_import_export'));
    $applntOrg_branch = $app->request->post('applntOrg_branch');
    $applntOrg_position = $app->request->post('applntOrg_position');
    $applntOrg_tel = $app->request->post('applntOrg_tel');
    $applntOrg_fax = $app->request->post('applntOrg_fax');
    $applntOrg_web = $app->request->post('applntOrg_web');
    $applntOrg_country_id = $app->request->post('applntOrg_country_id');
    $applntOrg_address = $app->request->post('applntOrg_address');
    $applntOrg_prov_id = $app->request->post('applntOrg_prov_id');
    $applntOrg_zipcode = $app->request->post('applntOrg_zipcode');
    $applntOrg_mobile_code = $app->request->post('applntOrg_mobile_code');
    $applntOrg_mobile_country = $app->request->post('applntOrg_mobile_country');
    ...
    $complnt_web = $app->request->post('complnt_web');
    $complnt_contact_address = $app->request->post('complnt_contact_address');
    $complnt_prov_id = $app->request->post('complnt_prov_id');
    $complnt_country_id = trim($app->request->post('complnt_country_id'));
    $complnt_zipcode = $app->request->post('complnt_zipcode');
    $complnt_mobile_code = $app->request->post('complnt_mobile_code');
    $complnt_mobile_country = $app->request->post('complnt_mobile_country');
    ...
    if(sizeof($complnt_file) != 0) {
        if($_FILES['complnt_file']['name'] != ['']) {
            $totalFiles = count($_FILES['complnt_file']['name']);
        }
    }

    $caseid = $db->createComplaint($app->request->post(),$compType_id,$compTypeSub1_id,$compTypeSub2_id,$case_status,$case_assign_status,$caseCh_id,$case_priority,$case_receivedoc_real_datetime, $case_disKPI_status,$caseDtl_title,$prodType_id,$caseDtl_derivation,$caseDtl_damage_val,$curren_id,$caseDtl_complnt_need,
...
$app->post('/caseDetail_BW', function() use ($app) {
    ...
    foreach ($id as $key => $val_id) {
        $case = array();
        $result = $db->getCaseDetail($val_id);
        $case["corporate_number"] = $val_id;
        $case["detail"] = array();
        if($result != NULL) {
            if($type == 1) {
                while ($res = $result->fetch_assoc()) {
                    $detail = array();
                    $detail["case_id"] = $res["case_id"];
                    if($res["company_type"] == 1) {
                        $company_type = 'ผู้ร้องเรียน';
                    } else {
                        $company_type = 'ผู้ถูกร้องเรียน';
                    ...
                    $detail["complaint_type"] = $res["compTypeSub2_name"];
                    if($res["reliable"] == 1) {
                        $reliable = 'เฝ้าระวัง (Watchlist)';
                    } else if($res["reliable"] == 2) {
                        $reliable = 'แบล็กลิสต์ (Blacklist)';
                    ...
                    $detail["company_status"] = $reliable;
                    $detail["case_create_datetime"] = $res["case_create_datetime"];
                    $detail["case_close_datetime"] = $res["case_close_datetime"];
                    array_push($case["detail"], $detail);
                }
            }else if($type == 2) { 
                while ($res = $result->fetch_assoc()) {
                    $detail = array();
            ...
            $case["detail"]  = "ไม่พบข้อมูล";
        }
        array_push($response["res_result"], $case);
    }
...
$app->post('/checkmember_ditp', function() use ($app) {
    $db = new DbHandler();
    $result = $db->getCaseAllYear(2023);
    $array_data = array();
    if($result != NULL) {
        while ($res = $result->fetch_assoc()) {
            $temp_data = array();
            $temp_data['case_id'] = $res['case_id'];
            $temp_data['member_ditp'] = "";
            $textId = str_replace("-","",$res['complnt_trade_number']);
            $textId = $db->data_filter($textId);
            $ditp_info = array();
            if(is_numeric($textId) && $textId!="" && $textId!="1111111111111" && $textId!= 112990024557){
                $ditp_info_res = checkmember($textId);
                if($ditp_info_res["res_code"]=="00"){
                    if (strcasecmp($res["complnt_valid_ditp_org"],$ditp_info_res["memberType"]) == 0) {
                        $temp_data['member_ditp'] = true;
                    }else{
                        $temp_data['member_ditp'] = false;
                    }
                    
                }else{
                    if ($res["complnt_valid_ditp_org"] == null) {
                        $temp_data['member_ditp'] = true;
                    }else{
                        $temp_data['member_ditp'] = false;
                    }
...
function verifyRequiredParamsRegis($required_fields) {
    ...
    $response['company_taxid'] = "เลขนิติบุคคล";
...
function checkmember($id_val){
    ...
    if($resp->code == 200){
        ...
        $retarr=array("res_code"=>"01","res_text" =>"NOT FOUND");
        return $retarr;
    }
